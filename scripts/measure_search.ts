...
import { CURATED_KINGDOMS, registerCuratedKingdoms } from '../test/sim/kingdoms';
    ...
    for (const ochre of BASELINE_STRATEGIES) {
      for (const indigo of BASELINE_STRATEGIES) {
        ...
        const result = runMatch({
          kingdomId: kingdom.id, seed, firstPlayerId: 'ochre', swapSides: false,
...
console.log(`kingdoms=${CURATED_KINGDOMS.length} seeds=${SEEDS.length} strategies=${BASELINE_STRATEGIES.length}`);
