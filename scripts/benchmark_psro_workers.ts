...
const mode = option('mode', 'score-only'), kingdomId = option('kingdom', 'deep-beam-tuning-009');
const turnLimit = positive('turn-limit', 50);
...
if (!kingdom) throw new Error(`Unknown benchmark kingdom ${kingdomId}.`);
...
try {
  ...
  const results = await evaluateCandidates(candidates, opponents, schedule, runner, {
    kingdomId: kingdom.id, turnLimitPerPlayer: turnLimit, actionCapPerTurn: 200,
  ...
  process.stdout.write(`${JSON.stringify({ schemaVersion: 1, benchmark: 'fixed-reservoir-psro-workers',
    mode, kingdomId, turnLimit, workers, candidates: candidateCount, blocks,
    matches: results.reduce((sum, entry) => sum + entry.matches, 0),
