...
export function renderStrategyReport(model: StrategyReportModel): string {
  ...
  const offerCounts = Object.values(model.evidence.variableCardOfferCounts);
  const offerRange = offerCounts.length ? `${Math.min(...offerCounts)}${Math.min(...offerCounts) === Math.max(...offerCounts) ? '' : `–${Math.max(...offerCounts)}`}` : '0';
...
export function generateStrategyReport(
  ...
  const tuningSize = balanceSuite.manifest.splits.find((split) => split.name === 'tuning')!.size;
  if (input.kingdoms.length !== tuningSize) throw new Error(`Expected ${tuningSize} tuning kingdoms, found ${input.kingdoms.length}.`);
  ...
  const tuningDesign = balanceSuite.manifest.splits.find((split) => split.name === 'tuning')!.design;
  if (variableCounts.some((count) => count < tuningDesign.cardCountMinimum || count > tuningDesign.cardCountMaximum)) {
    throw new Error('The tuning corpus does not match manifest card exposure.');
  }
...
if (entry && import.meta.url === pathToFileURL(entry).href) {
  try {
    ...
    process.stdout.write(`Wrote ${output ?? path.join(process.cwd(), '.html', 'strategy-report.html')} from ${model.evidence.kingdoms} tuning kingdoms.\n`);
  } catch (error) {
