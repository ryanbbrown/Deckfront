...
class TurnTools:
    ...
    def specs(self) -> list[ToolSpec]:
        return [
            ...
            ToolSpec(
                "enter_buy_phase",
                "Close board actions and auto-play treasures. Rejected when an available point was missed.",
                EmptyArgs,
...
def system_prompt() -> str:
    return "\n".join([
        "You play one side of Hexdeck as well as you can.",
        "Your objective is to win. Take a match-winning move whenever one exists.",
        "Score the maximumPointsAvailable before entering the buy phase.",
        ...
        "The strategy text controls choices among equally scoring lines and guides purchases.",
        "Use baseline moves and any number of action cards when useful.",
        "When board actions are complete, enter the buy phase, buy or skip, then commit the turn.",
...
def run_fake_model(tools: TurnTools, briefing: dict[str, Any]) -> str:
    if int(briefing.get("maximumPointsAvailable", 0)) != 0:
        raise RuntimeError("The fake model supports only fixtures without an available point.")
