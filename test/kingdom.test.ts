...
import {
  ...
  applyCommand, assertInvariants, checkInvariants, createGame, kingdomMarket, kingdomOf, listLegalActions,
...
describe('kingdom registry', () => {
  ...
  it('starts with three Scrap and also sells Scrap for 1 money', () => {
    ...
    expect(CARDS.scrap!.cost).toBe(1); expect(state.supply.scrap).toBe(ALWAYS_AVAILABLE_COUNT);
    state.phase = 'buy'; state.players.ochre.money = 1;
    const buy = listLegalActions(state).find((entry) => entry.label === 'Buy Scrap');
    expect(buy).toBeDefined();
    const bought = applyCommand(state, buy!.command);
    expect(bought.supply.scrap).toBe(ALWAYS_AVAILABLE_COUNT - 1);
    expect(bought.players.ochre.purchases).toEqual(['scrap']);
  ...
  it('limits starting builds to sold cards and the universal cards', () => {
    ...
    expect(() => submitStartingBuild(state, 'ochre', ['aim','cull','step','scrap'])).not.toThrow();
