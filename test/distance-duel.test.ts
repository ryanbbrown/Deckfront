...
describe('starting build', () => {
  it('creates no physical cards or draws before both hidden builds complete', () => {
    ...
    expect(state.nextCardSerial).toBe(21); expect(state.fighters.ochre).toMatchObject({ position: 2, health: 40 }); expect(state.fighters.indigo).toMatchObject({ position: 3, health: 40 });
