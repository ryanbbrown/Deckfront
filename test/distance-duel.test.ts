...
describe('starting build', () => {
  it('creates no physical cards or draws before both hidden builds complete', () => {
    ...
    expect(state.nextCardSerial).toBe(21); expect(state.fighters.ochre).toMatchObject({ position: 2, health: 40 }); expect(state.fighters.indigo).toMatchObject({ position: 3, health: 36 });
...
describe('cards and conditions', () => {
  ...
  it('Feint reapplies without stacking, while Drive moves both fighters or collides with a wall', () => {
    ...
    expect(state.fighters.indigo.health).toBe(35); expect(state.fighters.indigo.position).toBe(2); expect(state.fighters.ochre.position).toBe(2); expect(rangeBand(state)).toBe('Close'); expect(state.fighters.indigo.exposed).toBe(false);
