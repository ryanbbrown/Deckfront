...
describe('local GameService', () => {
  ...
  it('starts draft-off immediately with Scrap and rejects build commands', async () => {
    ...
    expect(view.cards.scrap).toMatchObject({ name:'Scrap', cost:1 }); expect(view.fixedCardIds).toContain('scrap'); expect(view.supply.scrap).toBe(10); expect(view.variableCardIds).not.toContain('scrap');
