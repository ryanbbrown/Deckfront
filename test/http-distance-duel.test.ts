...
describe('distance duel HTTP interface', () => {
  ...
  it('returns a specific old-save schema error through HTTP', async () => {
    const { base, games } = await server(); const id = '11111111-1111-4111-8111-111111111111'; await mkdir(games, { recursive: true }); await writeFile(path.join(games, `${id}.json`), JSON.stringify({ schemaVersion: 3 }));
    const response = await fetch(`${base}/api/games/${id}`); expect(response.status).toBe(409); expect(await response.json()).toEqual({ error: 'Saved game schema 3 is not supported. Start a new game.' });
  });
  ...
  it('revision-locks starting-build updates and exposes only redacted exports', async () => {
    ...
    const exported = await fetch(`${base}/api/games/${created.id}/export`).then((response) => response.json()) as Record<string, unknown>; expect(exported.schemaVersion).toBe(4); expect(JSON.stringify(exported)).not.toContain('committedState');
  });
});
