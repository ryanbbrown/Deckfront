...
describe('ThinHarness bridge schemas and trace privacy', () => {
  it('returns a validated fake build and action without persisting private briefing zones', async () => {
    ...
    try {
      const service = new GameService(new FileGameRepository(path.join(directory, 'games'))); const created = await service.create({ seed: 4, firstPlayerId: 'indigo', strategyPresetId: 'ranged-setup', strategyMarkdown: '# ranged' });
      ...
      const build = await runner.run(await service.getRecord(created.id)); expect(build).toMatchObject({ schemaVersion: 2, kind: 'build', definitionIds: ['footwork', 'aim', 'volley'] });
      ...
      const action = await runner.run(await service.getRecord(created.id)); expect(action).toMatchObject({ schemaVersion: 2, kind: 'action' });
      const trace = await readFile(action.tracePath!, 'utf8'); expect(trace).not.toContain('drawContentsUnordered'); expect(trace).not.toContain('"hand"'); expect(human.completedBuilds).toBeNull();
