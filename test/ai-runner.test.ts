...
describe('ThinHarness bridge schemas and trace privacy', () => {
  it('returns a validated fake build and action without persisting private briefing zones', async () => {
    ...
    try {
      ...
      const build = await runner.run(await service.getRecord(created.id)); expect(build).toMatchObject({ schemaVersion: 2, kind: 'build', definitionIds: ['footwork', 'footwork', 'aim', 'volley'], summary: 'Built the requested strategy package.' });
