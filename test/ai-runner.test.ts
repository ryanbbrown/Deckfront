...
import { DEFAULT_KINGDOM_ID, createGame, kingdomMarket, marketCost } from '../src/game';
import { ThinHarnessAiRunner, chooseFakeBuild } from '../src/server/aiRunner';
...
describe('ThinHarness bridge schemas and trace privacy', () => {
  it('returns a validated fake build and action without persisting private briefing zones', async () => {
    ...
    try {
      const service = new GameService(new FileGameRepository(path.join(directory, 'games'))); const created = await service.create({ seed: 4, firstPlayerId: 'indigo', strategyPresetId: 'ranged-setup', strategyMarkdown: '# edited trace strategy sentinel' });
      ...
      const build = await runner.run(await service.getRecord(created.id)); expect(build).toMatchObject({ schemaVersion: 2, kind: 'build', definitionIds: ['footwork', 'aim', 'volley'], summary: 'Built the requested strategy package.' });
  ...
  it('gives every shipped strategy preset a fake build the default kingdom sells within the budget', () => {
    const forSale = new Set(kingdomMarket(DEFAULT_KINGDOM_ID).map((card) => card.id));
    const state = createGame({ seed: 1 });
    for (const file of readdirSync(path.join(root, 'strategies')).filter((name) => name.endsWith('.md'))) {
      const presetId = file.replace(/\.md$/, '');
      const build = chooseFakeBuild({ strategy: { presetId } } as GameRecord);
      for (const definitionId of build) expect(forSale, `${presetId} builds ${definitionId}`).toContain(definitionId);
      expect(marketCost(state, build), `${presetId} build cost`).toBeLessThanOrEqual(12);
    }
  });
});
