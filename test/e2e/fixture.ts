...
export const test = base.extend<Fixtures>({
  ...
  baseUrl: async ({ dataDirectory }, use) => {
    const app = createHexdeckServer({ dataDirectory, strategyDirectory: path.join(root, 'strategies'), distDirectory: path.join(root, 'dist'), ai: { projectRoot: root, traceDirectory: path.join(dataDirectory, 'traces'), model: process.env.HEXDECK_AI_MODEL ?? 'openai:gpt-5.6-luna', effort: process.env.HEXDECK_AI_EFFORT ?? 'low', timeoutMilliseconds: 240_000, fakeModel: process.env.HEXDECK_E2E_LIVE !== '1' } });
  ...
  openGame: async ({ baseUrl, repository }, use) => {
    await use(async (page, mutate) => {
      ...
      await page.goto(baseUrl); await page.evaluate((id) => localStorage.setItem('hexdeck.activeGameId', id), created.id); await page.reload(); await expect(page.getByText(/your action/)).toBeVisible(); return record;
    });
  }
});
export { expect };
