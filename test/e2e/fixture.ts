...
export const test = base.extend<Fixtures>({
  ...
  baseUrl: async ({ dataDirectory }, use) => {
    const app = createHexdeckServer({ dataDirectory, distDirectory: path.join(root, 'dist') });
  ...
  openGame: async ({ baseUrl, repository }, use) => {
    await use(async (page, mutate) => {
      const response = await fetch(`${baseUrl}/api/games`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ seed: 7, firstPlayerId: 'ochre' }) });
...
export function resetRecord(record: GameRecord): void { record.initialState = cloneGame(record.state); record.committedCommands = []; record.undoCheckpoint = null; record.revision = 0; record.completedActions = 0; }
