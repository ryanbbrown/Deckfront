...
test('DD-E2E-031: visible market accepts the 10th Channel and disables the unavailable 11th', async ({ page, openGame }) => {
  await openGame(page); await page.route('**/api/games/*', async (route) => { if (route.request().method() !== 'GET') return route.continue(); const response = await route.fetch(); const view = await response.json() as { supply: Record<string, number> }; delete view.supply.channel; await route.fulfill({ response, json: view }); }); await page.reload(); await expect(page.locator('[data-market-card="Channel"]')).toContainText('×0'); await page.unroute('**/api/games/*');
