...
test('DD-E2E-001: full-table preview refreshes, explains, and keeps both local builds', async ({ page, baseUrl }) => {
  ...
  await page.getByLabel('Starting draft').check(); await page.getByRole('button', { name: 'Start game' }).click(); await expect(page.getByText('Player 1 starting build')).toBeVisible(); await expectTableWithinViewport(page); await page.locator('[data-market-card="Scrap"]').click(); await expect(page.getByRole('button', { name: 'Remove Scrap' })).toBeVisible(); await expect(page.getByTestId('build-budget')).toHaveText('0 / 12 · 3 carries'); await page.getByRole('button', { name: 'Remove Scrap' }).click(); await page.locator('[data-market-card="Copper"]').click(); await page.locator('[data-market-card="Copper"]').click(); await page.locator('[data-market-card="Step"]').click(); await expect(page.getByTestId('build-budget')).toHaveText('2 / 12 · 3 carries');
