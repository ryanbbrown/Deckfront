...
test('DD-E2E-006: Close Feint states its damage bonus and Drive moves both fighters', async ({ page, openGame }) => {
  ...
  await playCard(page, 'Feint'); await expect(page.locator('[data-player-score="indigo"]')).toContainText('Exposed'); await expect(page.locator('[data-player-score="indigo"]')).toHaveAccessibleName(/Exposed, Close-range attacks this turn: \+1 damage/);
