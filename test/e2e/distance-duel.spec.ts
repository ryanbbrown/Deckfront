...
test('DD-E2E-001: full-table preview refreshes, explains, and keeps both local builds', async ({ page, baseUrl }) => {
  ...
  await expect(page.getByRole('heading', { name: /^Battlefield \d+$/ })).toBeVisible(); await expect(page.locator('.pile-grid--fixed [data-market-card]')).toHaveCount(6); await expect(page.locator('.pile-grid--kingdom [data-market-card]')).toHaveCount(10); expect(await page.locator('.pile-grid--fixed [data-market-card]').evaluateAll((piles) => piles.map((pile) => pile.getAttribute('data-market-card')))).toEqual(['Copper','Scrap','Focus','Step','Silver','Gold']);
  ...
  const before = await page.locator('.pile-grid--kingdom [data-market-card]').allTextContents(); await page.getByRole('button', { name: 'Random battlefield' }).click(); const after = await page.locator('.pile-grid--kingdom [data-market-card]').allTextContents(); expect(after).not.toEqual(before);
...
test('DD-E2E-065: New game ignores late game responses and statistics', async ({ page, openGame }) => {
  ...
  await page.getByRole('checkbox', { name: 'Starting draft' }).uncheck();
