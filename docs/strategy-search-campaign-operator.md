...
The scheduler keeps at most two global worker waves ready. The execution state stores complete deterministic Goldfish partitions and adds jobs as workers consume the ready window. Matrix and PSRO add only the current score look and its reducer. Matrix and PSRO use four-core workers and choose task counts from measured scoring and coordination costs, bounded by `maxActiveCpus`. PSRO can split both candidate ranges and contiguous schedule-block ranges, then restores every candidate's scores in schedule order. Score containers stay warm across adjacent looks. A kingdom starts Matrix as soon as its reservoir validates and starts PSRO only after its complete 125-seed Matrix validates. No kingdom waits for another kingdom's stage.
...
- per-look task count, requested and running CPUs, worker duration distribution, queue delay, admission failures, coordination and reduction time, and total wall time;
...
For each evidence ID, the directory contains `report.json`, `goldfish/top-500000.hgf`, `goldfish/reservoir.hgf`, and the final Matrix and PSRO JSON evidence. `report.json.clientOperations` measures each final download and post-download validator by path, bytes, and wall time. Validated runtime chunks stay on the Volume until the publisher seals the whole kingdom completion receipt. Launch-scoped temporary files are removed during publication.

Completed PSRO evidence can contain unresolved candidate decisions. Uncertainty at a fixed look cap is not a failure. Missing, corrupt, failed, or interrupted work is incomplete. Analytics and balance reports run later and do not change strategy-search completion.
