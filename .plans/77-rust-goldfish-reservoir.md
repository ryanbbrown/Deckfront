...
Status: approved after plan review v2 (`.reviews/plans/rust-goldfish-reservoir/rust-goldfish-reservoir-synthesis-v2.md`).
...
- Header identity: the header carries the kingdom rule fingerprint and the shuffle seeds, not an evidence ID. The Rust binary cannot know a campaign evidence ID, and an evidence ID in the bytes would break local-versus-Modal byte identity. Evidence identity stays in publication receipts and paths, as today. Stage-two files and the reservoir carry the CRC-32 of the top file rows they were derived from, so a shard from another top file is rejected.
- Downstream strategy identity: the reservoir reader assigns `strategy.id = gf-<strategyNumber>`. Matrix and PSRO key candidates by that ID. The hashed `sg-` display ID is a 32-bit hash and can collide inside 20,000 rows; PSRO rejects duplicate candidate IDs, so the number is the only collision-free identity. `candidateIdentitiesValid` in `src/sim/thresholdRacingPsro.ts` accepts `gf-<n>` when `canonicalStrategy(candidateAt(n))` for the protocol kingdom equals the candidate canonical; it keeps accepting the `sg-` hash form only for the retained legacy PSRO experiment scripts.
...
- Test knobs: the reducers accept an explicit universe (`--start/--end`) and keep count (`--keep`) so a small local run can exercise the full pipeline. Production callers (the Python job, the Python `verify` calls, and the TypeScript campaign readers) pass none of these; the defaults are the full space, 500,000 top rows, and 20,000 reservoir rows, and `verify` and the TypeScript readers reject any other shape unless a test passes the same knobs explicitly. A test-sized file can never be published as evidence.
...
| 28 | 4 | source checksum: CRC-32 of the top file rows for kinds 2 and 4; 0 for kinds 1 and 3 |
| 32 | 16 | four seeds in ascending order; unused slots are 0; the seed count is implied by the kind |
...
- Stage-two rows: range over top-500000 row indexes (ranks). Row `i` holds the strategy number from top row `start + i` and the totals from the nine games. Seeds: `[4100001, 4100002, 4100003]`. Source checksum: the top file's row CRC-32.
- Top-500000: range `[0, 12972960)`, 500,000 rows, best first. Seeds: `[4100000]`.
- Reservoir: range `[0, 500000)` over top rows, 20,000 rows, best first. Seeds: all four. Source checksum: the top file's row CRC-32.

Result row (64 bytes): strategy number, then for each dummy opponent in the order stationary, chaser, kiter: games played, games that reached 50 damage, penalized turns to 50 (31 per unfinished game), damage total across 30 turns, money spent. This is the `OrderedProductProfileEvidence` field order in `src/sim/orderedGoldfishProduct.ts` (`trials`, `completions`, `penalizedTurnsTo50`, `damageArea`, `moneySpent`), not the Rust `Metrics` struct order.
...
`{ "command", "kingdomId", "rangeStart", "rangeEnd", "rowCount", "threads", "bytesRead", "bytesWritten", "elapsedMs", "scoringMs", "readMs", "writeMs", "reduceMs" }`. `scoringMs + readMs + writeMs + reduceMs` is at most `elapsedMs`. Result files never contain timings. A rule fingerprint longer than 16 bytes is an error; the writer never truncates it.
...
- `kingdom` prints `{ kingdomId, candidateCount, ruleFingerprint, cardIds }`. `strategies` prints one JSON line per number: `{ number, cardIds: [five IDs in buy order], counts: [five counts] }`. Tests and readiness use these.
...
- `score-two` verifies the top file header and CRC, reads rows `[start, end)`, plays nine games per strategy (seeds `4100001..4100003` × three profiles), and writes stage-two rows in rank order with the top file's row CRC as the source checksum.
- `reduce-two` reads the top file and the stage-two inputs, sorts inputs by range start, requires contiguous exact coverage of `[0, top row count)` with no overlap, requires every input's source checksum to equal the top file's row CRC and every row's strategy number to equal the top row at the same rank, adds the per-profile numbers, ranks, keeps the best `keep`, and writes the reservoir with both metric sets per row and the top file's row CRC as the source checksum.
- `verify` checks header fields, CRC, and row count for every kind. For `stage-one` it checks the expected range and that row `i` has number `start + i`. For `stage-two` it requires `--top` and checks the expected range, the source checksum, and that each row's number equals the top row at its rank. For `top` it checks the full range, 500,000 rows unless `--keep` is given, unique numbers inside the range, and best-first order. For `reservoir` it requires `--top` and checks 20,000 rows unless `--keep` is given, the source checksum, best-first combined order, unique numbers, and that each row's shuffle-1 numbers equal the top file's row for that strategy. It exits non-zero with a message on any difference and prints a JSON summary on success.
...
- Add `src/sim/goldfishReservoir.ts`: header constants, header decode, CRC check via `node:zlib.crc32`, `readGoldfishTop(file, kingdomId)` and `readGoldfishReservoir(file, kingdomId, { top? })`. Records carry `rank`, `strategyNumber`, `strategy` (from `candidateAt(number)` with `id` replaced by `gf-<strategyNumber>`), `displayId` (`strategy.id`), `canonicalStrategy`, `stageOne`, `additional`, `combined` (as `OrderedProductScoreEvidence`), and `stageOneRank` when the top file is given. Readers validate the fingerprint against `nativeRuleFingerprint(kingdomId, 30, 200)`, seeds, kind, CRC, the production row count and range (test overrides are explicit options), unique numbers, best-first order using `compareEvidence` plus the number tiebreak, and, when the top file is given, the source checksum and the shuffle-1 numbers.
- `src/sim/thresholdRacingPsro.ts`: `candidateIdentitiesValid` takes the protocol kingdom and accepts `gf-<n>` IDs whose `canonicalStrategy(candidateAt(n))` equals the candidate canonical, in addition to the `sg-` hash form used by the retained legacy PSRO experiment scripts.
...
- Update readers: `scripts/strategy_search_campaign_matrix_manifest.ts` (`reservoirIdentityHash` = reservoir file SHA-256, `reservoirContentHash` = top file SHA-256; strategies are the first 50 records), `scripts/strategy_search_campaign_parallel_psro.ts` init mode (`goldfishRank: rank`, `strategyId: strategy.id`, `seedSourceHash` and `sourceIdentityHash` from file SHA-256 values), `scripts/strategy_search_campaign_psro.ts` (`rankedSha256` becomes the top file SHA-256; `entries` are the new records), and `scripts/strategy_search_validate_artifact.ts` (`goldfish-one-reduce` reads and validates the top file; `goldfish-two-reduce` reads the reservoir with the top cross-check; `psro` candidate IDs come from the reservoir records). Drop the evidence-ID header checks; the fingerprint check replaces them. Post-download deep validation uses these TypeScript readers; publication validation on Modal uses only the Rust `verify` command.
- `src/sim/strategySearchCampaign.ts`: replace `orderedProduct` in `KingdomEvidenceIdentity` with `goldfish: { generator: 'ordered-five-rung-v1', rowFormat: 'goldfish-rows-v1', scorerVersion, candidateCount, retainedCount, reservoirCount, profiles, seeds, cardIds }`. No traversal, no collision allowance. Evidence IDs change; there is no compatibility requirement. `src/sim/strategySearchCampaignOperator.ts` reads `kingdom.goldfish.candidateCount` and `kingdom.goldfish.retainedCount`; `test/sim/strategySearchCampaign.test.ts` asserts the new shape.
- Delete `validateCampaignGoldfishStage` from `src/sim/strategySearchStages.ts` and its test block in `test/sim/orderedGoldfishProduct.test.ts`; keep `createCampaignStageControlMarker`. Remove the `orderedGoldfishSplitArtifact` import and cases from that test file, the `nativeKingdom009` import and case from `test/sim/nativeStrategySearch.test.ts`, and the stale `strategy_search_goldfish.ts` reference from `test/sim/strategySearchSubprocess.test.ts`. Prune exports in `src/sim/orderedGoldfishBenchmark.ts` that lose every caller (`parseOrderedGoldfishArgs`, the CLI default constants, `representativeCandidateIndices` if unused, `candidateChecksumFromIterable`).
...
- Update `strategy-search-image-files.json` and `strategy-search-scientific-files.json`: remove the deleted files, add `rust/goldfish/kingdoms.json`, `rust/goldfish/src/reservoir.rs`, and `src/sim/goldfishReservoir.ts`. The image copies only allowlisted `rust/` files before `cargo build`, so every Rust source must be listed.

## Python changes (`modal/`)

- `strategy_search_goldfish_job` launches the Rust binary directly through `_strategy_search_run_subprocess`: `score-one`/`score-two` with `--start --end --threads <cpu> --out --report`, `reduce-one`/`reduce-two` with `--inputs` (the JSON list Python already writes) and `--top` where needed. No `--evidence-id`. It maps the Rust report into the existing phase keys per command: `score-one`/`score-two`: `scoringMs` → `scoringMs`, `readMs + writeMs` → `intermediateSerializationAndReadMs`; `reduce-one`: `readMs` → `intermediateSerializationAndReadMs`, `reduceMs` → `reductionComputeMs`, `writeMs` → `finalTop500000WriteMs`; `reduce-two`: the same with `finalTop20000WriteMs`. Every other phase key is 0 and the remainder of `elapsedMs` goes to `orchestrationQueueMs`, so the controller's phase-sum invariant holds.
- `_strategy_search_validate_publication` for `goldfish-one`, `goldfish-two`, `goldfish-one-reduce`, and `goldfish-two-reduce` runs `hexdeck-goldfish verify` with the expected kind and range, and passes `--top` (the published top file) for `goldfish-two` and `goldfish-two-reduce`. No knobs. `_strategy_search_validate_compact` and its `struct` parsing are removed.
...
- The legacy ordered-product launcher is deleted from `modal/native_strategy_search.py`: `score_shard`, `product_search`, `ordered_product_stage_one`, `ordered_product_stage_two`, `_run_product_stage`, `ordered_product_controller`, `controller`, the `launch` local entrypoint, `validate_launch_limits`, the ordered-product authorization tables and the authorization/continuation branch inside `reserve_cost`, `_native_shard`, `_run_rust`, `_ordered_product_cli`, `_valid_ordered_product_checkpoint`, `_preserve_corrupt_file`, `_campaign_stage_one_ranges`, `_remaining_stage_seconds`, `_valid_stage_one_chunk_metadata`, `projected_product_cost_usd`, `projected_ordered_product_cost_usd`, `_result_hash`, `valid_result`, `_result_path`, `_stable_hash`, `ordered_subprocess_timeouts`, and every test that exists only for them. Keep what the competitive entrypoints reach: `reserve_cost` (without the authorization branch), `claim_controller`, `update_run_status`, `record_controller_call`, the cost-ledger constants, `projected_cost_usd`, `MAX_PHYSICAL_CORES`, `_run_checked`, `_called_process_details`, `_atomic_json`, the image definition, the competitive functions and entrypoints (`competitive_*`, `stage_competitive_input`, `launch_competitive`, `run_competitive`, and their helpers), and every `strategy_search_*` function. Delete constants that lose every reference.
...
- header round trip; `verify` rejects a flipped row byte, a wrong range, a wrong fingerprint, a wrong seed set, a wrong kind, and a row-count mismatch;
- `reduce-one` rejects a coverage gap, an overlap, a duplicate range, wrong seeds, a wrong fingerprint, and a wrong kind;
- `reduce-two` and `verify --kind stage-two --top` reject a stage-two file with a correct CRC but a wrong strategy number at one rank, and a file with a wrong source checksum;
- every kingdom in `kingdoms.json` compiles and derives 12,972,960 candidates.
...
- small pipeline: `score-one` for `[0, 600)` as one range, as `[0,200)+[200,600)`, and as `[0,300)+[300,600)`; `reduce-one --keep 120`; `score-two` split two ways; `reduce-two --keep 40`; `verify` passes; `top` and `reservoir` bytes are identical across the three layouts and across two runs; the TypeScript readers decode the files, check order, and reconstruct strategies that match `candidateAt` with `gf-<number>` IDs;
- TypeScript reader rejections: a flipped row byte, a wrong fingerprint, a wrong seed set, a wrong source checksum, and a `--keep`-shortened file offered on the production path;
- the PSRO init adapter produces unique `gf-` candidate IDs and `candidateIdentitiesValid` accepts them for the kingdom and rejects a wrong canonical;
- `kingdoms.json` is current;
- existing conformance tests keep passing.

Python (`npm run modal:test`): goldfish job command shape and report mapping (patched subprocess), the mapped phases satisfy the controller phase-sum invariant for all four commands, validation calls `verify` with the expected arguments including `--top`, readiness uses the Rust `kingdom` command and fails when the binary is missing, controller materialization tests unchanged, competitive entrypoints still import and reserve cost, and the ordered-product launcher tests are removed with the launcher.
