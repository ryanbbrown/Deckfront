...
The first complete `npm test` run passed 82 files and 758 tests. Two tests in `test/sim/randomPsro.test.ts` failed. The resumability test exceeded its default five-second limit under full-suite load; three focused repeats then passed in 2.865, 2.884, and 2.984 seconds. The applicable suite passed all 82 other files and 742 tests, then passed 17 of the 18 random-PSRO tests. This distinguishes the timeout from a strategy-search regression.
...
| Failures leave false state and omit retry cost | Fixed. Startup and controller failures persist as failed state. Every submitted, admission-failed, worker-failed, and successful attempt remains in state and contributes to the resource-cost report. |
