...
- `fetch_file(volume, remote, local, expected_size, sleep=time.sleep) -> {bytes, wallMs, attempts}`: streams `volume.read_file(remote)` into `local`, retries per D6, raises on a size mismatch, and skips the size check when `expected_size` is `None`.
...
- `download(config)`: D7 through `fetch_files`. Result: `artifacts` (per file: `kingdomId`, `path`, `relative`, `bytes`, `wallMs`), `kingdoms` (per kingdom: `kingdomId`, `files`, `bytes`), `bytes`, `wallMs`, `concurrency`.
- `status(state_file)`: D8.

### `modal/strategy_search_runtime.py`

- `_download_final_artifacts` uses `fetch_files` with concurrency 16, the two Goldfish sizes, and `None` for the full route's JSON evidence files, and keeps the per-artifact `{evidenceId, path, bytes, wallMs}` entries and totals. The import is inside the function (D10).
...
4. Paid PSRO smoke, execution `b695aa14…`, the same 8 kingdoms at 16 cores and 128 CPUs: 8 of 8 complete with two clean full searches (0–3 admissions each), makespan 155 s, job wall 16–147 s, parallel download of 314 files in 8.7 s (about 97 s at the previous 0.31 s per file), structural validation valid for 8 of 8 in 1.2 s, measured cost $0.1066 against a $20 guard; deploy 7.3 s, readiness 15.0 s, launch 77.3 s. The launch step still checks inputs and uploads Matrix files one kingdom at a time (about 10 s per kingdom) and is a follow-up outside this plan.
