...
A fresh default `psro` run does not replay its final evidence semantically. It writes `decisions.hpd` from live records and structurally checks the file against its checkpoint reference. A resume from a complete checkpoint rebuilds the decisions through bounded replay and compares the existing file with that payload. Deliberate `psro-verify` independently replays decisions and races, verifies look evidence, checks the scientific file set and final references, and verifies expanded Matrix files. It is opt-in locally through `download --verify`, and it stays off in the Modal job. This matches the project rule that deep Goldfish, Matrix, and PSRO verification is opt-in. Update the sentences that say ordinary reporting trusts "completed deep verification": `docs/strategy-search-evidence.md` (the HST backfill paragraph), `docs/strategy-search-process.md` under "Evidence and restart", and the two README sentences about backfill and reporting. They must say that evidence is structurally checked and that deep verification is a separate, deliberate `psro-verify` run.
