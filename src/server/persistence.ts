...
export class FileGameRepository implements GameRepository {
  ...
  async create(record: GameRecord): Promise<void> {
    ...
    try {
      await readFile(this.pathFor(record.id), 'utf8');
      throw new Error(`Game already exists: ${record.id}`);
    } catch (error) {
  ...
  async load(id: string): Promise<GameRecord> {
    try {
      const raw = JSON.parse(await readFile(this.pathFor(id), 'utf8')) as unknown;
      const record = gameRecordSchema.parse(raw) as GameRecord;
