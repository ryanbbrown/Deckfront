...
interface Job { baseRevision: number; status: 'running' | 'complete' | 'error'; game?: SafeGameView; error?: string }
export class AiTurnCoordinator {
  ...
  async start(id: string): Promise<AiTurnStatus> {
    ...
    const job: Job = { baseRevision: record.revision, status: 'running' }; this.jobs.set(id, job); void this.run(id, job); return { status: 'running' };
  }
  async status(id: string): Promise<AiTurnStatus> {
    ...
    const record = await this.service.getRecord(id);
    if (job.game?.revision === record.revision) { this.jobs.delete(id); return { status: 'complete', game: job.game }; }
  ...
  private async run(id: string, job: Job): Promise<void> {
    ...
    try {
      while (true) {
        const record = await this.service.getRecord(id);
        if (record.state.activePlayerId !== record.aiPlayerId || record.state.winner) { job.game = await this.service.getRecord(id).then(() => job.game!); break; }
        if (record.state.phase === 'startingBuild') {
          ...
          try { job.game = await this.service.commitAiBuild(id, result.baseRevision, result.definitionIds, result.summary, result.durationSeconds); await this.runner.finalize?.(result, { status: 'complete', committedRevision: job.game.revision }); }
        ...
        if (decisions >= 30) {
          ...
          job.game = await this.service.commitAiAction(id, record.revision, phaseEnd.id, 'Decision limit reached; ended phase.', 0, decisions, true); decisions += 1; continue;
        }
        const result = await this.runner.run(record);
        if (result.kind !== 'action') throw new ConflictError('AI bridge returned a build during a normal turn.');
        try { job.game = await this.service.commitAiAction(id, result.baseRevision, result.actionId, result.summary, result.durationSeconds, decisions); await this.runner.finalize?.(result, { status: 'complete', committedRevision: job.game.revision }); }
        catch (error) { await this.finishError(result, error); throw error; }
        decisions += 1;
      }
      if (!job.game) {
        const record = await this.service.getRecord(id); job.game = await this.service.getRecord(id).then(() => ({}) as SafeGameView);
        throw new Error(`AI completed without a safe game view at revision ${record.revision}.`);
      }
      job.status = 'complete';
    } catch (error) { job.status = 'error'; job.error = error instanceof Error ? error.message : 'AI decision failed.'; }
  }
