...
const EFFECT_MAP: Record<CardMechanic, CardEffect> = {
  ...
  feint: { ...BASE, tactical: true, gate: needsClose, command: (cardInstanceId) => ({ type: 'playFeint', cardInstanceId }),
    resolve: (context, values) => { context.draw(context.actorId, value(values, 'draw')); context.state.fighters[context.targetId].exposedAttacksRemaining = value(values, 'attacks'); context.record('condition', { condition: 'Exposed', change: 'set', targetId: context.targetId }); } },
  ...
  flurry: { ...BASE, tactical: true, command: (cardInstanceId) => ({ type: 'playFlurry', cardInstanceId }),
    resolve: (context, values) => { const tactical = context.state.turnState.cardsPlayed.slice(0, -1).filter((id) => EFFECTS[cardDefinition(id).mechanic].tactical).length; context.damage(context.targetId, tactical * value(values, 'perAction'), false); } },
  ...
  channel: { ...BASE, tactical: true, command: playAction, resolve: (context, values) => { context.gainMana(value(values, 'mana')); context.draw(context.actorId, value(values, 'draw')); } },
  ...
  prism: { ...BASE, tactical: true, command: playAction, resolve: (context, values) => { context.gainMana(value(values, 'mana')); context.draw(context.actorId, value(values, 'draw')); context.requestDiscard(value(values, 'discard')); } },
  ...
  attune: { ...BASE, tactical: true, command: playAction, resolve: (context, values) => { const copies = context.state.turnState.copiesPlayed.attune ?? 1; context.gainMana(value(values, 'mana') + (copies - 1) * value(values, 'perCopy')); context.draw(context.actorId, value(values, 'draw')); } },
  ...
  scrap: { ...BASE, tactical: false, command: playAction, resolve: (context, values) => context.damage(
