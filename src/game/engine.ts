...
function execute(state: GameState, command: GameCommand): void {
  switch (command.type) {
    ...
    case 'endBuyPhase': { const actor = state.activePlayerId; const player = state.players[actor]; player.deck.discard.push(...player.deck.hand, ...player.deck.play); player.deck.hand = []; player.deck.play = []; player.money = 0; player.mana = Math.min(player.mana, 3); player.firstBuyPending = false; player.firstBuyMoney = 0; state.fighters[actor].aimed = false; state.fighters[opponent(actor)].exposed = false; draw(state, actor, 5); state.turnState = emptyTurnState(); state.activePlayerId = opponent(actor); state.phase = 'action'; state.turn += 1; state.players[state.activePlayerId].positionChanged = false; record(state, 'turn', { turn: state.turn, activePlayerId: state.activePlayerId }); return; }
  }
}
