...
function dealDamage(state: GameState, targetId: PlayerId, base: number, close: boolean): void {
  const target = state.fighters[targetId]; const exposed = close && target.exposedAttacksRemaining > 0;
  const amount = base + (exposed ? (values(resolveCard(state, 'feint')).bonus ?? 0) : 0);
  if (exposed) target.exposedAttacksRemaining -= 1;
...
function rangedDamage(state: GameState, actorId: PlayerId, targetId: PlayerId, base: number): void {
  const fighter = state.fighters[actorId]; const amount = base + fighter.aimBonus;
  if (fighter.aimBonus) { fighter.aimBonus = 0; record(state, 'condition', { condition: 'Aimed', change: 'consumed', targetId: actorId }); }
...
function changeMana(state: GameState, playerId: PlayerId, amount: number, spent: boolean): void {
  ...
  if (amount < 0) player.carriedMana -= Math.min(player.carriedMana, -amount);
