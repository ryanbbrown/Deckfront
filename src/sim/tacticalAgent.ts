...
export function tacticalView(
  ...
  return {
    ...
    aimed: state.fighters[playerId].aimBonus > 0,
    aimBonus: state.fighters[playerId].aimBonus,
    opponentExposed: state.fighters[opponentId].exposedAttacksRemaining > 0,
    opponentExposedAttacksRemaining: state.fighters[opponentId].exposedAttacksRemaining,
    opponentExposedBonus: state.fighters[opponentId].exposedAttacksRemaining > 0 ? resolveCard(state, 'feint').values?.bonus ?? 0 : 0,
