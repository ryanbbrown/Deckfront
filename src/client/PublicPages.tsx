...
function RulesContent() {
  return <article className="public-article">
    ...
    <section><h2>Shared market</h2><p>Both players buy from the same 16 market piles. Copper, Silver, Gold, Step, Focus, and Scrap are always available. Ten more card piles change from match to match.</p></section>
    ...
    <section><h2>Attack families</h2><div className="rule-family-grid"><article className="rule-family rule-family--melee"><h3>Melee</h3><p>Melee attacks work at Close range.</p></article><article className="rule-family rule-family--ranged"><h3>Ranged</h3><p>Ranged attacks work at Near or Far range.</p></article><article className="rule-family rule-family--mana"><h3>Mana</h3><p>Mana attacks work at any range, but you must build and spend mana. You can carry up to 2 unspent mana between turns.</p></article></div></section>
...
function AboutContent() {
  return <article className="public-article">
    ...
    <section><h2>Initial public playtest</h2><p>You can play against an AI opponent or share one computer for a local match. The playtest has four AI difficulty levels, 46 card types, 160 selected markets, optional local starting drafts, persistent saves, and multi-step undo.</p></section>
    <section><h2>AI and balance</h2><p>Each AI strategy uses an ordered purchase plan that people can read, plus a shared policy for card play and movement. The work values strategies that are clear, repeatable, and useful across many markets rather than an opaque opponent that is hard to explain.</p><p>Repeated simulated matches help compare strategies and card use across the 160 selected markets. This work aims to keep Melee, Ranged, and Mana plans competitive and to give every card a useful place.</p></section>
    <section><h2>What is coming</h2><ul><li>Online multiplayer.</li><li>Player accounts, profiles, and public statistics.</li><li>New markets outside the selected 160, with matching AI opponents.</li><li>Mobile layouts.</li></ul></section>
  </article>;
}
