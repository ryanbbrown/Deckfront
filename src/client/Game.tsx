...
export function CardFace({ card, indicators }: { card: CardDefinition; indicators?: React.ReactNode }) { const copyLength = card.headline.length + (card.detail?.length ?? 0); const density = copyLength > 150 ? ' card__rules--very-dense' : copyLength > 90 ? ' card__rules--dense' : ''; return <>{indicators}<span className="card__header"><strong className="card__title">{card.name}</strong><span className="card__cost" aria-label={`Cost ${card.cost}`}>{card.cost}</span></span><span className="card__image" aria-hidden="true" /><span className={`card__rules${density}`}><strong className="card__headline">{card.headline}</strong>{card.detail ? <small className="card__detail">{card.detail}</small> : null}</span></>; }
