...
export function Game({ game, error, onGame, onError, onNew }: GameProps) {
  ...
  return <main className="table-shell">
    ...
    <section className="played-panel table-zone"><div className="zone-title"><h2>Played this turn</h2><span>Consecutive copies share a stack.</span></div><div className="played-row" data-testid="played-row">{playedGroups.length ? playedGroups.map((group) => { const pendingAction = pendingHandSelection?.choices.find((choice) => choice.cardInstanceId && group.instances.some((card) => card.id === choice.cardInstanceId)); return <PlayedCard key={group.instances[0]!.id} card={game.cards[group.definitionId]!} group={group} pendingAction={pendingAction} selected={pendingAction?.id === pendingHandActionId} busy={busy} onSelect={setPendingHandActionId} />; }) : <p className="empty-row">Cards move here from your hand.</p>}</div></section>
...
function PreviewArena() { return <section className="arena-zone table-zone"><div className="range-label">Near · 1 space</div><div className="arena" role="img" aria-label="Six space line arena">{[1,2,3,4,5,6].map((space) => <div className="arena-space" key={space}><span className="arena-space__number">{space}</span>{space === 3 ? <div className="fighter fighter--ochre">P1<small>37 HP</small></div> : null}{space === 4 ? <div className="fighter fighter--indigo">P2<small>40 HP</small></div> : null}</div>)}</div></section>; }
...
export function CardFace({ card }: { card: CardDefinition }) { const copyLength = card.headline.length + (card.detail?.length ?? 0); const density = copyLength > 150 ? ' card__rules--very-dense' : copyLength > 90 ? ' card__rules--dense' : ''; return <span className="card__face"><span className="card__header"><strong className="card__title">{card.name}</strong><span className="card__cost" aria-label={`Cost ${card.cost}`}>{card.cost}</span></span><span className="card__image" aria-hidden="true" /><span className={`card__rules${density}`}><strong className="card__headline">{card.headline}</strong>{card.detail ? <small className="card__detail">{card.detail}</small> : null}</span></span>; }
