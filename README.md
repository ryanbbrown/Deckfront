...
Draft-on decks start with 7 Copper. Up to 3 unspent starting money carries into the first Buy phase. Draft-off decks add 3 Scrap, have no carry, and skip the build. Only the first Scrap a player plays each turn deals its 1 damage. Scrap cannot be bought or gained. Starting-build cards do not reduce market piles. Mana gained during a player’s turn is available for that turn and the player’s next turn only. At most 3 mana carries past the end of a turn; unused carried mana expires at the end of the next turn.
