...
The server saves games in `.data/games` by default. You can set `PORT`, `HOST`, `HEXDECK_DATA_DIR`, or `HEXDECK_STATIC_DIR`. Saved game records, browser game views, and exports use schema version 14. The server rejects older saves and does not migrate them.
