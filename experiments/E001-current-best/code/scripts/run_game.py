...
def main() -> int:
    ...
    write_run_metadata(args, run_dir, players, runner="claude", drafts=drafts(args), status="running")
    started_sessions: set[str] = set()

    for _ in range(args.max_turns):
        timeline = read_json(run_dir / "timeline.json")
        if timeline.get("terminalWinEvents"):
            write_run_metadata(args, run_dir, players, runner="claude", drafts=drafts(args), status="complete")
        ...
        if result.returncode != 0:
            write_run_metadata(args, run_dir, players, runner="claude", drafts=drafts(args), status="invalid", error=result.stderr or result.stdout)
            print(result.stdout)
        ...
        if validation.returncode != 0:
            ...
            if args.retry_on_invalid > 0:
                ...
                if not retry_ok:
                    write_run_metadata(args, run_dir, players, runner="claude", drafts=drafts(args), status="invalid", error=validation.stderr or validation.stdout)
                    return 1
            else:
                write_run_metadata(args, run_dir, players, runner="claude", drafts=drafts(args), status="invalid", error=validation.stderr or validation.stdout)
        ...
        if not committed_expected_turn(timeline, previous_entries, turn_id, player.id):
            ...
            if args.retry_on_invalid > 0:
                ...
                if not retry_ok:
                    write_run_metadata(args, run_dir, players, runner="claude", drafts=drafts(args), status="invalid", error=error)
                    return 1
            else:
                write_run_metadata(args, run_dir, players, runner="claude", drafts=drafts(args), status="invalid", error=error)
                return 1
    ...
    write_run_metadata(args, run_dir, players, runner="claude", drafts=drafts(args), status="max_turns")
