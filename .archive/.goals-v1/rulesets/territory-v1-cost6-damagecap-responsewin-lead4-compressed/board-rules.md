# Territory V1 Cost 6 Damage Cap Response Win Lead 4 Compressed Income Rules

This is an income-compression variant of `territory-v1-cost6-damagecap-responsewin-lead4` for E015 playtests. It keeps the same cards, units, timing, recruit cost 6, deck-damage cap, and lead-4 response-window win condition, but changes board supply income to reduce automatic replacement at 4-4 center splits and reduce extra conversion from 6+ centers.
...
- Deck config: `rulesets/territory-v1-cost6-damagecap-responsewin-lead4-compressed/deck.yaml`
...
Do not use the older 7-Copper draft-start prose from prior rulesets for E015 runs. If a playtest uses a different initial setup, record that in the run notes and downgrade that run's evidence quality.

## Win Condition

A player wins at the beginning of their turn if they have a confirmed 4-unit lead.
...
This is compressed center income. Compared with `2 + controlled centers`, it lowers normal 4-4 replacement from 6 income to 5 income, gives a 5-center player exactly enough income to recruit one unit, and caps the extra income from 6+ centers.
...
With compressed center income and recruits costing 6 supply:

| Center split | Income A | Income B | Recruit pace |
| --- | ---: | ---: | --- |
| 4 / 4 | 5 | 5 | Neither player recruits every turn from income alone; kills should matter longer. |
| 5 / 3 | 6 | 4 | Leader recruits each turn; behind player needs savings. |
| 6 / 2 | 6 | 3 | Leader no longer gains extra income beyond the 5-center pace. |
| 7 / 1 | 7 | 2 | Leader saves slowly; behind player is under heavy pressure. |
| 8 / 0 | 7 | 1 | Full control is strong but does not create near-multi-recruit pacing. |
