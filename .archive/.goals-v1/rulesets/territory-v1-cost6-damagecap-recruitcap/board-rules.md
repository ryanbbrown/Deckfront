# Territory V1 Cost 6 Damage Cap Recruit Cap Rules

This is an anti-snowball variant of `territory-v1-cost6-damagecap` for E009 playtests. It keeps the same cards, units, map, timing, recruit cost 6, and deck-damage cap, but limits each player to at most one recruited unit per turn so center-income leads cannot convert into sudden multi-body jumps.
...
- Deck config: `rulesets/territory-v1-cost6-damagecap-recruitcap/deck.yaml`
...
Players may recruit at most one unit per turn, even if they can pay for more and have multiple legal spawn spaces.

Saved supply still carries over. A player with a center lead can recruit more consistently, but cannot convert saved supply into multiple bodies in a single board phase.
...
With income equal to `2 + controlled supply centers`, recruits costing 6 supply, and a maximum of one recruit per turn:

| Center split | Income A | Income B | Recruit pace |
| --- | ---: | ---: | --- |
| 4 / 4 | 6 | 6 | Each player can recruit 1 unit per turn with no surplus. |
| 5 / 3 | 7 | 5 | Leader recruits each turn and banks surplus; behind player alternates unless already saved. |
| 6 / 2 | 8 | 4 | Leader recruits each turn but excess supply waits; behind player needs savings. |
| 7 / 1 | 9 | 3 | Leader still cannot recruit more than once per turn; behind player slows sharply. |
| 8 / 0 | 10 | 2 | Leader banks large surplus but cannot create an immediate multi-unit swing. |
