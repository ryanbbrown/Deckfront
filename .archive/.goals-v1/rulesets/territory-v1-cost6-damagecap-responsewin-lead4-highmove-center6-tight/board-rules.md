# Territory V1 Cost 6 Damage Cap Response Win Lead 4 High Movement Center 6 Tight Rules

This is a high-movement anti-grind variant of `territory-v1-cost6-damagecap-responsewin-lead4` for E024b playtests. It keeps the same cards, timing, recruit cost 6, normal income, deck-damage cap, high movement values, and lead-4 response-window unit-count win condition. It keeps E024's late six-center dominance response-window win, but tightens the unit-deficit clear condition so the controller cannot be 2 or more living units behind.

## Source Files

- Map: `maps/sketch-v5-recenter.json`
- Sample run state: `.games/territory-v1-playtest/board.json`
- Units: `rulesets/territory-v1-cost6-damagecap-responsewin-lead4-highmove-center6-tight/units.json`
- Deck config: `rulesets/territory-v1-cost6-damagecap-responsewin-lead4-highmove-center6-tight/deck.yaml`
...
The initial deck is fixed for this high-movement six-center-tight variant: each player starts with 7 Coppers plus up to 12 coin to draft additional starting cards before turn 1. Unspent draft coin does not carry over in E024b runs.
...
2. If the active player controls at least 6 of the 8 supply centers and is not behind by 2 or more living units, record a pending six-center dominance threat for that player if one is not already pending.
3. If the active player already had that same pending six-center dominance threat after the opponent's previous completed turn, and still controls at least 6 centers while not behind by 2 or more living units, the active player wins immediately.
4. The opponent gets a full turn to clear the threat by reducing the threatened player's control below 6 centers or moving at least 2 living units ahead.
...
- P2 later starts a turn still controlling 6 centers and not behind by 2 or more units after P1 failed to clear the prior pending threat: P2 wins.
- P2 starts a turn controlling 6 centers while down 8 units to 10: no pending six-center dominance threat is created because P2 is behind by 2 living units.
...
Units start at their max HP and use the high-movement values from `rulesets/territory-v1-cost6-damagecap-responsewin-lead4-highmove-center6-tight/units.json`.
