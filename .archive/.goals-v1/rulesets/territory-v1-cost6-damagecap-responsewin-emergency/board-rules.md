# Territory V1 Cost 6 Damage Cap Response Win Emergency Rules

This is a response-comeback variant of `territory-v1-cost6-damagecap-responsewin` for E011 playtests. It keeps the same cards, units, map, timing, recruit cost 6, deck-damage cap, and response-window win timing, but adds a narrow emergency defender recruit while a player is answering a pending unit-count loss.
...
- Deck config: `rulesets/territory-v1-cost6-damagecap-responsewin-emergency/deck.yaml`
...
- If the active player is currently responding to an opponent's pending unit-count win threat, they may recruit one emergency defender this turn for 3 supply instead of 6.
- The emergency defender must be a `guardian`, `scout`, or `healer`.
- The emergency defender still enters in an empty home-base hex and is delayed like any other recruit.
- The emergency discount applies to only one recruited unit during that response turn. Additional recruits that turn cost 6 supply each.
- The emergency defender is allowed only if it can help reduce or prevent the pending 3-unit lead by the next start-of-turn check. If the active player is not facing a pending unit-count loss, all recruits cost 6.
- If the active player has less than 3 supply, they cannot use the emergency defender rule.
