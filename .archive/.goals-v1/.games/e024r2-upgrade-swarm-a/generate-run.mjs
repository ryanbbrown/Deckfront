...
const TITLE = 'E024 R2 upgrade/support vs swarm/economy A strict high-movement six-center dominance';
...
const starterBoard = JSON.parse(await readFile(join(ROOT, '.games', 'e024-highmove-center6-starter.board.json'), 'utf8'));
...
async function writeNotes() {
  ...
  const lines = [
    '# E024 R2 Upgrade/Support vs Swarm/Economy A',
    '',
    ...
    '',
    'Legality notes:',
    '- Generated from the assigned E024 high-movement six-center dominance starter with explicit strict action logs for movements, recruits, attacks, healing, and permanent upgrades.',
    ...
    winner
      ? '- The important tension was whether late six-center dominance could resolve a support-versus-swarm replacement loop below 50 turns and whether P1 could clear by flipping a center or moving 3 units ahead.'
      : '- The important tension was whether late six-center dominance could resolve a support-versus-swarm replacement loop below 50 turns and whether P1 could clear by flipping a center or moving 3 units ahead.',
    '',
    'Win-window log:',
    ...notes.map((note) => `- ${note}`)
...
function hasSixCenterThreat(state, player) {
  return countCenters(state, player) >= 6 && countUnits(state, player) >= countUnits(state, other(player)) - 2;
}
