...
const runDir = '.games/e014a-center-vs-flank-b';
...
const centerPlans: Record<Player, string[]> = {
  P1: ['center-center-north', 'center-center', 'center-east', 'center-southeast', 'center-northeast', 'center-center-south'],
  P2: ['center-west-south', 'center-center-south', 'center-southeast', 'center-east', 'center-center', 'center-northwest', 'center-center-north']
};

const recruitPlans: Record<Player, UnitType[]> = {
  P1: ['guardian', 'marksman', 'healer', 'druid', 'guardian', 'marksman', 'druid', 'healer'],
  P2: ['scout', 'raider', 'marksman', 'guardian', 'scout', 'raider', 'marksman', 'guardian']
};

const p1BuyPlan = ['silver', 'peddler', 'village', 'silver', 'armory', 'training', 'gold', 'smithy', 'healer', 'second-wind', 'peddler', 'training'];
const p2BuyPlan = ['silver', 'peddler', 'village', 'silver', 'gold', 'blast', 'peddler', 'village', 'storm', 'gold', 'silver', 'second-wind'];
...
function chooseUpgradeTarget(board: BoardState, player: Player, kind: 'health' | 'damage'): Unit | undefined {
  const preferred = player === 'P1'
    ? ['guardian', 'marksman', 'healer', 'druid', 'raider', 'scout']
    : ['marksman', 'guardian', 'raider', 'scout', 'druid', 'healer'];
...
function recruit(board: BoardState, map: any, player: Player, completedTurn: number): string[] {
  ...
  const maxRecruits = player === 'P1' ? 2 : 2;
...
function chooseBuy(state: GameState, legal: LegalAction[], player: Player): number {
  ...
  for (const cardId of plan) {
    const max = player === 'P1'
      ? (cardId === 'armory' || cardId === 'training' ? 2 : cardId === 'second-wind' || cardId === 'healer' ? 2 : 4)
      : (cardId === 'gold' ? 3 : cardId === 'blast' || cardId === 'storm' || cardId === 'second-wind' ? 2 : cardId === 'peddler' || cardId === 'village' ? 4 : 6);
  ...
  for (const fallback of player === 'P1' ? ['gold', 'armory', 'training', 'healer', 'second-wind', 'peddler', 'village', 'smithy', 'silver'] : ['gold', 'blast', 'storm', 'peddler', 'village', 'silver']) {
...
function chooseDeckAction(state: GameState, legal: LegalAction[], player: Player, bought: string[]): number {
  ...
  if (state.phase === 'action') {
    ...
    const priorities = player === 'P1'
      ? ['Play Village', 'Play Peddler', 'Play Training', 'Play Armory', 'Play Second Wind', 'Play Smithy', 'Play Bandage', 'Play Potion', 'Play Healer', 'Play Zap']
      : ['Play Village', 'Play Peddler', 'Play Blast', 'Play Storm', 'Play Second Wind', 'Play Smithy', 'Play Bandage', 'Play Zap', 'Play Potion'];
...
function applyBoardTurn(board: BoardState, map: any, hexes: Set<string>, player: Player, produced: Record<string, number>, completedTurn: number): { summary: string; reasoning: string } {
  ...
  const reasoning = `${player === 'P1'
    ? 'P1 followed the center-control assignment by buying reliable economy/support, recruiting guardians/marksmen/healers, and trying to anchor center-north, center, east, and southeast.'
    : 'P2 followed the flank/economy assignment by using mobile units to pressure lower and side centers while adding enough payload to punish isolated center units.'} Board events: ${events.join('; ')}.`;
...
async function main(): Promise<void> {
  ...
  const blocked = new Set<string>(map.blocked.map((coord: Coord) => key(coord)));
  const hexes = new Set<string>(map.hexes.filter((coord: Coord) => !blocked.has(key(coord))).map((coord: Coord) => key(coord)));
  const timeline: { schemaVersion: 1; title: string; entries: ReplayEntry[] } = {
    schemaVersion: 1,
    title: 'E014a center-vs-flank B tempo replay',
    entries: []
  };
  seedCounters(board);
  ...
  const resultTakeaway = winner === 'P1'
    ? `- P1's center-control plan held enough of the central/east anchor line to deny the 6-2 flank economy and convert support buys plus durable recruits into a confirmed lead-4 win.`
    : winner === 'P2'
      ...
      : `- No winner emerged within the extended cap. The final board still had live units and contested centers, but neither side showed a plausible near-term forced lead-4 conversion, so this run is unresolved stall evidence.`;
  const notes = `# E014a Center vs Flank B Notes
