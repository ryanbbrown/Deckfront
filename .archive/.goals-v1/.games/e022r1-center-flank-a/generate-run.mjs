...
const timeline = {
  schemaVersion: 1,
  title: 'E022 round 1 center-control vs flank/economy high-movement strict A',
  entries: []
};
...
for (let turnIndex = 1; turnIndex <= 90; turnIndex += 1) {
  ...
  timeline.entries.push({
    ...
    reasoning: reasoningFor(active, turnIndex, actions, counts, centerSplit)
  });
}

if (!winner) {
  throw new Error('run did not reach a legal lead-4 response-window winner within 90 turns');
}
...
await writeFile(join(root, 'notes.md'), notesFor(winner, completedTurns, board, winLog));
...
function reasoningFor(player, turnIndex, actions, counts, centerSplit) {
  const plan =
    player === 'P1'
  ...
  return `${plan} ${churn} Unit counts after turn: P1 ${counts.P1}, P2 ${counts.P2}. Centers after turn: P1 ${centerSplit.P1}, P2 ${centerSplit.P2}, neutral ${centerSplit.neutral}.`;
}

function notesFor(finalWinner, turns, state, log) {
  const counts = countUnits(state);
  const centers = countCenters(state);
  return `# E022r1 Center-Flank A
...
Winner: ${finalWinner} by confirmed lead-4 response-window unit-count win after ${turns} completed player turns.
...
High movement produced faster center contact and more recapture attempts than the low-movement branch. In this generated line, P2 held a 4-3 center edge at the end, but P1's center-control posture and combat trades produced the confirmed unit-count win.
`;
}
