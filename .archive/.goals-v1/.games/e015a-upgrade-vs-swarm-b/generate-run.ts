...
const runDir = '.games/e015a-upgrade-vs-swarm-b';
...
const recruitPlans: Record<Player, UnitType[]> = {
  ...
  P2: ['scout', 'raider', 'marksman', 'guardian', 'scout', 'marksman', 'raider', 'guardian']
};

const p1BuyPlan = ['silver', 'village', 'armory', 'training', 'peddler', 'smithy', 'second-wind', 'armory', 'training', 'healer', 'gold', 'second-wind'];
const p2BuyPlan = ['silver', 'peddler', 'village', 'silver', 'gold', 'peddler', 'village', 'gold', 'silver', 'peddler', 'smithy'];
...
function compressedIncome(centers: number): number {
  const table = [1, 2, 3, 4, 5, 6, 6, 7, 7];
  return table[centers] ?? 7;
}
...
function recruit(board: BoardState, map: any, player: Player, completedTurn: number): string[] {
  ...
  if (player === 'P1' && supply < 10 && liveUnits(board, player).length >= 7) {
    return recruited;
  }
...
function chooseBuy(state: GameState, legal: LegalAction[], player: Player): number {
  ...
  for (const cardId of plan) {
    const max = player === 'P1'
      ? (cardId === 'armory' || cardId === 'training' ? 4 : cardId === 'second-wind' ? 3 : 4)
...
function applyBoardTurn(board: BoardState, map: any, hexes: Set<string>, player: Player, produced: Record<string, number>, completedTurn: number): { summary: string; reasoning: string } {
  ...
  const income = compressedIncome(centersControlled(board, player));
...
async function main(): Promise<void> {
  ...
  const timeline: { schemaVersion: 1; title: string; entries: ReplayEntry[] } = {
    schemaVersion: 1,
    title: 'E015a upgrade/support vs swarm/economy B compressed replay',
    entries: []
  };
  seedCounters(board);
  ...
  const resultTakeaway = winner === 'P1'
    ...
    : winner === 'P2'
      ? `- P1's upgrades concentrated value onto durable guardians and ranged support, but the quality posture could not keep pace once P2 converted center breadth into enough replacement bodies despite compressed income.`
  ...
  const notes = `# E015a Upgrade/Support vs Swarm/Economy B Notes
