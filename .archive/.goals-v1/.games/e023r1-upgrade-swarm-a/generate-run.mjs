...
const TITLE = 'E023 R1 upgrade/support vs swarm/economy A strict high-movement late center-majority';
...
const starterBoard = JSON.parse(await readFile(join(ROOT, '.games', 'e023-highmove-centermid-starter.board.json'), 'utf8'));
...
for (let turnNumber = 1; turnNumber <= 160; turnNumber += 1) {
  ...
  const centerEvent = checkCenterMajorityThreat(board, player, turnNumber, completedTurns, startEvents);
  if (centerEvent?.winner) {
    winner = centerEvent.winner;
    winnerType = 'center-majority';
    notes.push(`${winner} confirmed a pending late center-majority threat at the start of turn ${turnNumber}; no extra board phase was recorded.`);
    break;
  }
...
if (!winner && completedTurns >= 40) {
  notes.push(`Stopped unresolved after ${completedTurns} completed player turns: no pending unit-count or late center-majority threat survived the response window, home-base congestion limited further recruitment, and repeated legal attacks/healing no longer changed the unit-count or center-control gap enough to create a plausible forced finish.`);
}
...
function checkCenterMajorityThreat(state, player, turnNumber, turnsCompleted, startEvents) {
  if (pendingCenterThreat) {
    const pendingPlayer = pendingCenterThreat.player;
    const stillThreatening = hasCenterMajorityThreat(state, pendingPlayer);
    if (pendingPlayer === player) {
      if (stillThreatening) {
        return { winner: player };
      }
      const event = `${pendingPlayer}'s pending late center-majority threat was cleared before turn ${turnNumber}: ${countCenters(state, pendingPlayer)} centers and unit count ${countUnits(state, pendingPlayer)} vs ${countUnits(state, other(pendingPlayer))}.`;
      notes.push(event);
      startEvents.push(event);
  ...
  if (!pendingCenterThreat && turnsCompleted >= 24 && hasCenterMajorityThreat(state, player)) {
    pendingCenterThreat = { player, afterTurn: turnsCompleted };
    const event = `${player} created a pending late center-majority threat at the start of turn ${turnNumber}: ${countCenters(state, player)} centers and unit count ${countUnits(state, player)} vs ${countUnits(state, other(player))}.`;
    notes.push(event);
    startEvents.push(event);
  }
...
async function writeNotes() {
  ...
  const lines = [
    '# E023 R1 Upgrade/Support vs Swarm/Economy A',
    '',
    `Result: ${winner ? `${winner} wins by confirmed ${winnerType === 'center-majority' ? 'late center-majority response-window win' : 'lead-4 response-window unit-count win'}` : 'unresolved'}.`,
    ...
    '',
    'Legality notes:',
    '- Generated from the assigned E023 high-movement late center-majority starter with explicit strict action logs for movements, recruits, attacks, healing, and permanent upgrades.',
    ...
    winner
      ? '- The important tension was whether late center-majority could resolve a support-versus-swarm replacement loop before it became a long pure unit-count grind.'
      : '- The important tension was whether late center-majority could resolve a support-versus-swarm replacement loop before it became a long pure unit-count grind.',
    '',
    'Win-window log:',
    ...notes.map((note) => `- ${note}`)
...
function hasCenterMajorityThreat(state, player) {
  return countCenters(state, player) >= 5 && countUnits(state, player) >= countUnits(state, other(player));
}
