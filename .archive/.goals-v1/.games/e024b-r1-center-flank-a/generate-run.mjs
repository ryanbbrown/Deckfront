...
const timeline = {
  schemaVersion: 1,
  title: 'E024b round 1 center-control vs flank/economy high-movement six-center-tight strict A',
  entries: []
};
...
for (let turnIndex = 1; turnIndex <= 90; turnIndex += 1) {
  ...
  if (
    ...
    (startCenters[pendingSixCenterPlayer] < 6 || startCounts[nextPlayer(pendingSixCenterPlayer)] - startCounts[pendingSixCenterPlayer] >= 2)
  ) {
    winLog.push(
  ...
  if (completedTurns >= 18 && startCenters[active] >= 6 && startCounts[opponent] - startCounts[active] < 2) {
...
function notesFor(finalWinner, finalWinType, turns, state, log) {
  ...
  return `# E024b R1 Center-Flank A
