...
const root = '.games/e024r2-center-flank-a';
const ruleset = 'territory-v1-cost6-damagecap-responsewin-lead4-highmove-center6';
const mapId = 'sketch-v5-recenter';
const starterPath = '.games/e024-highmove-center6-starter.board.json';
...
const timeline = {
  schemaVersion: 1,
  title: 'E024 round 2 center-control vs sharper flank/economy high-movement six-center strict A',
  entries: []
};
...
let pendingSixCenterPlayer = null;
let winner = null;
let winType = null;
...
for (let turnIndex = 1; turnIndex <= 90; turnIndex += 1) {
  ...
  const opponent = nextPlayer(active);
  const startCounts = countUnits(board);
  const activeLead = startCounts[active] - startCounts[opponent];
  const startCenters = countCenters(board);

  if (pendingWinPlayer && startCounts[pendingWinPlayer] - startCounts[nextPlayer(pendingWinPlayer)] < 4) {
    winLog.push(`Start turn ${turnIndex}: ${pendingWinPlayer}'s pending lead-4 threat cleared at units P1 ${startCounts.P1} / P2 ${startCounts.P2}.`);
    pendingWinPlayer = null;
  }
  if (
    pendingSixCenterPlayer &&
    (startCenters[pendingSixCenterPlayer] < 6 || startCounts[nextPlayer(pendingSixCenterPlayer)] - startCounts[pendingSixCenterPlayer] >= 3)
  ) {
    winLog.push(
      `Start turn ${turnIndex}: ${pendingSixCenterPlayer}'s pending six-center threat cleared at centers P1 ${startCenters.P1} / P2 ${startCenters.P2}, units P1 ${startCounts.P1} / P2 ${startCounts.P2}.`
    );
    pendingSixCenterPlayer = null;
  }
  ...
  if (activeLead >= 4) {
    if (pendingWinPlayer === active) {
      winner = active;
      winType = 'unit-count';
      winLog.push(`Start turn ${turnIndex}: ${active} confirms lead-4 response-window win at units P1 ${startCounts.P1} / P2 ${startCounts.P2}.`);
      break;
    }
    pendingWinPlayer = active;
    winLog.push(`Start turn ${turnIndex}: ${active} creates pending lead-4 threat at units P1 ${startCounts.P1} / P2 ${startCounts.P2}.`);
  }

  if (completedTurns >= 18 && startCenters[active] >= 6 && startCounts[opponent] - startCounts[active] < 3) {
    if (pendingSixCenterPlayer === active) {
      winner = active;
      winType = 'six-center-dominance';
      winLog.push(
        `Start turn ${turnIndex}: ${active} confirms late six-center dominance at centers P1 ${startCenters.P1} / P2 ${startCenters.P2}, units P1 ${startCounts.P1} / P2 ${startCounts.P2}.`
      );
      break;
    }
    pendingSixCenterPlayer = active;
    winLog.push(
      `Start turn ${turnIndex}: ${active} creates pending late six-center dominance threat at centers P1 ${startCenters.P1} / P2 ${startCenters.P2}, units P1 ${startCounts.P1} / P2 ${startCounts.P2}.`
    );
  }
  ...
  timeline.entries.push({
    ...
    reasoning: reasoningFor(active, completedTurns, actions, counts, centerSplit, winLog)
  });
}
...
function moveUnits(state, player, turnIndex, actions) {
  ...
  for (const unit of movable) {
    ...
    const target = targetFor(unit, turnIndex);
...
function targetFor(unit, turnIndex) {
  ...
  const p2FlankTargets = [
    ...
    centerById.get('center-center'),
    centerById.get('center-northwest')
  ];
  ...
  if (unit.type === 'marksman') {
    return centerById.get(turnIndex % 3 === 0 ? 'center-center-south' : 'center-east');
  }
  return centerById.get(turnIndex % 3 === 0 ? 'center-center-south' : 'center-southeast');
}
...
function recruitUnits(state, beforeState, player, turnIndex, actions) {
  const order =
    player === 'P1'
      ...
      : ['scout', 'raider', 'marksman', 'scout', 'raider', 'guardian'];
  ...
  const desiredRecruits = player === 'P2' && playerUnits <= opponentUnits + 4 ? 2 : 1;
  ...
  return {
    ...
    game: {
      ...
      players: [
        ...
        { id: 'P2', deck: ['copper', 'copper', 'copper', 'copper', 'copper', 'zap', 'bandage', 'peddler', 'silver', 'gold'] }
      ],
      trash: [],
      supply: []
    }
...
function reasoningFor(player, completed, actions, counts, centerSplit, log) {
  const plan =
    player === 'P1'
      ? 'P1 keeps a center-control posture, using high-movement scouts and raiders to recapture contested centers while guardians and marksmen anchor the middle.'
      : 'P2 keeps the flank/economy posture, using high-movement scouts and raiders to pressure lower/east lanes and convert center income into repeated recruitment.';
  const churn =
  ...
  const sixCenterRule =
    completed < 18
      ? `The late six-center dominance rule is still gated because only ${completed} completed player turns exist after this entry.`
      : 'The late six-center dominance rule is live at future start-of-turn checks; pending, cleared, or confirmed six-center threats are recorded in the win log when conditions change.';
  return `${plan} ${churn} Unit counts after turn: P1 ${counts.P1}, P2 ${counts.P2}. Centers after turn: P1 ${centerSplit.P1}, P2 ${centerSplit.P2}, neutral ${centerSplit.neutral}. ${sixCenterRule} Latest response-window state: ${recentWinState}`;
}

function notesFor(finalWinner, finalWinType, turns, state, log) {
  const counts = countUnits(state);
  const centerCounts = countCenters(state);
  return `# E024r2 Center-Flank A
...
Final living units: P1 ${counts.P1}, P2 ${counts.P2}. Final center split: P1 ${centerCounts.P1}, P2 ${centerCounts.P2}, neutral ${centerCounts.neutral}.
...
The late six-center dominance rule was checked only at start-of-turn after at least 18 completed player turns had already been recorded. Pending, cleared, and confirmed six-center threats are recorded in the win log when they occur.
...
High movement again produced fast center contact and repeated recapture attempts. This round intentionally sharpened P2's flank/economy pressure to test whether six-center dominance adds territorial pressure without producing cheap early wins.
`;
}
