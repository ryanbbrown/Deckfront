...
const root = '.games/e015b-rush-vs-engine-b';
...
const boardPlans: BoardPlan[] = [
  ...
  {
    summary: 'P1 captured center-north and flipped southeast, but compressed income delayed the first recruit.',
    reasoning:
      'The scout had not reached center-east before income, so P1 had only five supply and could not recruit. Blast damage was not assigned because no ready attack line was concrete enough after movement.',
    moves: [
      ['P1-raider-1', 5, 3],
      ['P1-scout-1', 8, 7],
      ['P1-guardian-1', 8, 3],
  ...
  {
    summary: 'P2 stepped guardian-2 toward center but could not yet afford the healer.',
    reasoning:
      'P2 could not let P1 sit on five centers uncontested. Compressed income left only three supply after the earlier guardian buy, so the support engine had to wait.',
    moves: [
      ['P2-guardian-2', 1, 8],
      ['P2-scout-1', 5, 7],
      ['P2-druid-1', 4, 6],
  ...
  {
    summary: 'P1 killed the wounded P2 scout and bought a marksman instead of a scout.',
    reasoning:
      'Storm/Zap damage was only counted where it rode on concrete attacks. P1 focused the damaged scout with scout and marksman pressure, then added ranged pressure because the delayed recruit curve made every body choice tighter.',
    moves: [
      ['P1-scout-1', 9, 7],
      ['P1-raider-1', 6, 4],
      ['P1-raider-2', 8, 1],
  ...
  {
    summary: 'P2 began walking guardian-2 toward the center lane and finally added the healer.',
    reasoning:
      'After losing scout-1, P2 stopped chasing the southeast scout and instead saved enough compressed income to add the first healer.',
    moves: [
      ['P2-guardian-2', 2, 8],
      ['P2-druid-1', 5, 6],
      ['P2-marksman-1', 5, 8]
  ...
  {
    summary: 'P1 upgraded raider-1, damaged the center guardian, and added a delayed scout.',
    reasoning:
      'Training was tied to the raider that could keep converting attacks through healing. Compressed income kept this to one new mobile body, not a wider swarm.',
    moves: [
      ['P1-raider-1', 6, 4],
      ['P1-raider-2', 6, 2],
      ['P1-marksman-1', 6, 3]
  ...
  {
    summary: 'P1 killed marksman-1 and used the five-center income to add a third raider.',
    reasoning:
      'P1 shifted from the dead upgraded raider into replacement melee pressure. The five-center compressed income still supported one recruit per turn, but without much saved overflow.',
    moves: [
      ['P1-raider-2', 5, 3],
      ['P1-scout-2', 9, 2],
      ['P1-marksman-2', 9, 0]
  ...
  {
    summary: 'P2 repaired the center defender and bought a delayed guardian screen.',
    reasoning:
      'P2 was losing unit parity, but the engine package still bought time by healing guardian-2 and finally reaching enough saved supply for another guardian.',
    moves: [
      ['P2-guardian-2', 4, 8],
      ['P2-healer-1', 2, 8]
    ],
  ...
  {
    summary: 'P1 added a marksman and wounded healer-1 to weaken future stabilization.',
    reasoning:
      'The rush avoided dumping attacks into a fully healed guardian. Hitting the healer made P2’s engine less able to erase chip damage while P1 chose ranged reach over a heavier holder.',
    moves: [
      ['P1-scout-2', 8, 4],
      ['P1-guardian-1', 6, 4],
      ['P1-scout-1', 9, 6],
  ...
  {
    summary: 'P2 killed scout-1 but could not afford a replacement scout.',
    reasoning:
      'P2 found the best response to the count pressure by removing a mobile flipper. The kill mattered, but compressed income left no immediate replacement body.',
    moves: [
      ['P2-guardian-3', 1, 8],
      ['P2-druid-1', 5, 7],
      ['P2-healer-1', 3, 8]
  ...
  {
    summary: 'P1 recruited a guardian and kept center-east pressure instead of diving the bunker.',
    reasoning:
      'P1 was ahead, but not by enough for a pending threat. The better rush line was to preserve attackers, keep the five-center income pattern alive, and add a durable response-window body.',
    moves: [
      ['P1-raider-2', 6, 5],
      ['P1-raider-3', 7, 2],
      ['P1-guardian-1', 5, 4],
  ...
  {
    summary: 'P2 stabilized guardian-2 again but had no affordable scout flip.',
    reasoning:
      'P2’s engine still had counterplay through healing, but the compressed economy removed the old replacement scout that would have threatened northwest income denial.',
    moves: [
      ['P2-guardian-3', 2, 8],
      ['P2-druid-1', 5, 7]
    ],
  ...
  },
  {
    ...
    reasoning:
      'Start-of-turn count was P1 8 to P2 4, creating a pending lead-4 threat. After the kill and recruit, P1 ended at 9 to 3, forcing P2 to answer before P1’s next start.',
    moves: [
      ['P1-scout-2', 8, 6],
      ['P1-guardian-2', 10, 2],
      ['P1-raider-3', 6, 4],
    ...
    damage: [['P2-scout-2', 8]],
    recruit: ['raider'],
    note: 'Pending P1 lead-4 threat recorded at the beginning of turn 19; P1 ended the turn with 9 units to P2 3.'
  },
  {
    summary: 'P2 killed raider-3 and recruited once, but the pending threat stayed live.',
    reasoning:
      'P2 found a necessary kill and could afford one guardian, but compressed income left no second recruit. The count moved from P1 9 / P2 3 to P1 8 / P2 4, so the P1 lead stayed exactly four.',
    moves: [
      ['P2-guardian-3', 3, 7],
      ['P2-druid-1', 5, 6]
    ],
    ...
    recruit: ['guardian'],
    note: 'P2 failed to clear the turn-19 pending P1 lead; P1 wins at the beginning of turn 21.'
  },
  {
...
async function main(): Promise<void> {
  ...
  const config = await loadGameConfig('rulesets/territory-v1-cost6-damagecap-responsewin-lead4-compressed/deck.yaml');
  ...
  board.notes = ['E015b B replay start: compressed-income starter board with fixed deck.yaml starting decks.'];

  const timeline = {
    schemaVersion: 1 as const,
    title: 'E015b rush-vs-engine B compressed replay',
    entries: [] as unknown[]
  };
...
function addIncome(board: BoardState, player: Player): void {
  ...
  const compressedIncome = [1, 2, 3, 4, 5, 6, 6, 7, 7][centers];
  if (compressedIncome === undefined) {
    throw new Error(`Unsupported center count for compressed income: ${centers}`);
  }
  supply.amount += compressedIncome;
}
...
function notesMarkdown(finalBoard: BoardState): string {
  ...
  return `# E015b Rush vs Engine B Replay Notes
