...
const root = '.games/e019r1-center-flank-a';
...
const scripts = [
  ...
  },
  {
    ...
    attacks: [
      ...
      { attacker: 'P2-marksman-1', target: 'P1-guardian-1', deckDamage: 1 }
    ],
  ...
  },
  {
    ...
    recruits: [{ unit: 'P1-marksman-3', type: 'marksman', at: [12, 1] }]
  },
  {
    player: 'P2',
    bought: ['gold'],
...
const timeline = {
  schemaVersion: 1,
  title: 'E019 round 1 center-control vs flank strict A',
  entries: []
};
...
for (let index = 0; index < scripts.length; index += 1) {
  ...
  const summary = summarizeTurn(id, turn, counts, centerSplit);
...
function summarizeTurn(id, turn, counts, centerSplit) {
  const combat = (turn.attacks ?? []).length === 0 ? 'no attacks' : `${turn.attacks.length} logged attacks`;
  const recruits = (turn.recruits ?? []).length === 0 ? 'no recruits' : `${turn.recruits.length} recruits`;
