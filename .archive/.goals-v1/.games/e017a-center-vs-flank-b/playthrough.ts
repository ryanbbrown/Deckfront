...
const RUN = '.games/e017a-center-vs-flank-b';
...
const p1Targets: Record<UnitType, string[]> = {
  scout: ['center-southeast', 'center-east', 'center-center', 'center-center-north', 'center-northeast'],
  raider: ['center-center-north', 'center-center', 'center-east', 'center-southeast', 'center-northeast'],
  guardian: ['center-center-north', 'center-center', 'center-east', 'center-southeast', 'center-northeast'],
  marksman: ['center-center-north', 'center-center', 'center-east', 'center-southeast', 'center-northeast'],
  druid: ['center-center', 'center-center-north', 'center-east', 'center-southeast', 'center-northeast'],
  healer: ['center-center-north', 'center-center', 'center-east', 'center-southeast', 'center-northeast']
};
const p2Targets: Record<UnitType, string[]> = {
  scout: ['center-west-south', 'center-center-south', 'center-northwest', 'center-southeast', 'center-center'],
  raider: ['center-center-south', 'center-west-south', 'center-southeast', 'center-center', 'center-northwest'],
  marksman: ['center-west-south', 'center-center-south', 'center-northwest', 'center-center', 'center-southeast'],
...
const p1Recruits: UnitType[] = ['guardian', 'marksman', 'healer', 'guardian', 'druid', 'marksman', 'guardian', 'marksman', 'healer'];
const p2Recruits: UnitType[] = ['scout', 'raider', 'scout', 'marksman', 'guardian', 'raider', 'marksman', 'scout', 'guardian'];
...
async function main(): Promise<void> {
  ...
  board.notes = [...(board.notes ?? []), 'E017a center-vs-flank-b generated full-game run; normal 2 + centers income used every board phase.'];
  ...
  for (let turnNumber = 1; turnNumber <= maxTurns && !winner; turnNumber += 1) {
    ...
    const centerGateOpen = entries.length >= 16;
    if (centerGateOpen && centers[player] >= 5 && counts[player] - counts[opponent] >= 2) {
      ...
      notes.push(`Turn ${turnNumber} ${player} start: ${player} created pending tight center-control threat with ${centers[player]} centers and unit count ${counts[player]}-${counts[opponent]} after ${entries.length} completed player turns.`);
      ...
      if (threatCenters < 5 || counts[threatened] - counts[defender] < 2) {
        notes.push(`Turn ${turnNumber} ${player} start: pending ${threatened} center-control threat cleared at centers P1 ${centers.P1} / P2 ${centers.P2} and units P1 ${counts.P1} / P2 ${counts.P2}.`);
        pendingCenter = null;
      }
    }
  ...
  notes.unshift(`# E017a Center vs Flank B Notes`, '');
  ...
  if (entries.length > 40) {
    notes.push('- Continued past 40 completed player turns because the position was not stalled: P1 created a center-control threat at the checkpoint, P2 answered it, and the unit-lead response window then resolved the game.');
  }
  ...
  notes.push('- Evidence quality: full if validator passes; tactical legality was generated with movement, range, occupancy, normal income, response-window threats, and recruit-cost checks.');
  ...
  notes.push('- Tight center-control was gated until at least 16 completed timeline entries, then required 5+ centers and a 2-unit living-unit lead.');
  ...
  await writeJson(join(RUN, 'timeline.json'), { schemaVersion: 1, title: 'E017a Center vs Flank B', entries });
...
function moveUnits(board: BoardState, player: Player): string[] {
  ...
  for (const unit of units) {
    ...
    let best = responseWindowMove(board, player, unit, candidates) ?? centerControlConversionMove(board, player, unit, candidates) ?? unit as Coord;
...
function responseWindowMove(board: BoardState, player: Player, unit: Unit, candidates: Coord[]): Coord | undefined {
  const opponent = player === 'P1' ? 'P2' : 'P1';
  if (pendingCenter !== opponent) {
    return undefined;
  }
  const candidateKeys = new Set(candidates.map(key));
  const centerPriority = player === 'P1'
    ? ['center-east', 'center-southeast', 'center-center', 'center-center-north', 'center-center-south', 'center-northeast', 'center-northwest', 'center-west-south']
    : ['center-center', 'center-center-north', 'center-east', 'center-northeast', 'center-southeast', 'center-center-south', 'center-northwest', 'center-west-south'];
  for (const centerId of centerPriority) {
    const control = board.supplyControl.find((center) => center.id === centerId);
    const coord = centerCoords[centerId];
    if (control?.controller === opponent && coord && candidateKeys.has(key(coord))) {
      return coord;
    }
  }
  return undefined;
...
function scoreCoord(board: BoardState, player: Player, unit: Unit, coord: Coord, targets: string[]): number {
  ...
  for (let index = 0; index < targets.length; index += 1) {
    ...
    score = Math.min(score, distance(coord, target) * 10 + index * 25 + controlPenalty + opponentBonus + neutralBonus);
  }
  return score + (enemyNear ? -2 : 0);
}
