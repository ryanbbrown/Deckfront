...
const runDir = '.games/e015a-center-vs-flank-a';
...
const starterBoardPath = '.games/e015-compressed-starter.board.json';
...
function compressedIncome(centerCount: number): number {
  const table = [1, 2, 3, 4, 5, 6, 6, 7, 7];
  return table[centerCount] ?? 7;
}
...
function applyBoardTurn(
  ...
  const controlledBeforeIncome = centersControlled(board, player);
  const income = compressedIncome(controlledBeforeIncome);
  const events: string[] = [`compressed income ${income} from ${controlledBeforeIncome} centers`];
...
async function main(): Promise<void> {
  ...
  const timeline: { schemaVersion: 1; title: string; entries: ReplayEntry[] } = {
    schemaVersion: 1,
    title: 'E015a center vs flank A compressed replay',
    entries: []
  };
  seedCounters(board);
  ...
  const resultTakeaway = winner === 'P1'
    ...
    : winner === 'P2'
      ? '- P2 still converted flank economy into the decisive body lead, but compressed income made the six-center phase produce one recruit per turn rather than extra snowball supply.'
      : '- Compressed income softened both the 4-4 replacement grind and the 6-2 flank spike, but neither side created a confirmed lead before the cap.';
  const notes = `# E015a Center vs Flank A Replay Notes
