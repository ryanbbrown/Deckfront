...
const root = '.games/e019r1-rush-engine-a';
const map = JSON.parse(await readFile('maps/sketch-v3-contest.json', 'utf8'));
const initialBoard = JSON.parse(await readFile('.games/e013-contest-starter.board.json', 'utf8'));

const unitRules = {
  guardian: { attack: 4, hp: 16, movement: 1, range: 1 },
  raider: { attack: 6, hp: 8, movement: 2, range: 1 },
  marksman: { attack: 4, hp: 8, movement: 1, range: 2 },
  scout: { attack: 2, hp: 8, movement: 3, range: 2 },
  druid: { attack: 4, hp: 10, movement: 1, range: 1, heal: 1 },
  healer: { attack: 1, hp: 4, movement: 1, range: 2, heal: 1 }
};
...
const buyPlans = {
  P1: [
    ...
    { drawnHand: ['zap', 'zap', 'copper', 'copper', 'rest'], played: ['zap', 'zap'], bought: ['silver'], produced: { damage: 2 } },
    { drawnHand: ['blast', 'copper', 'copper', 'copper', 'bandage'], played: ['blast'], bought: ['storm'], produced: { damage: 2 } },
    { drawnHand: ['storm', 'zap', 'silver', 'copper', 'rest'], played: ['storm', 'zap'], bought: ['second-wind'], produced: { damage: 3, stormTargets: 2 } },
    { drawnHand: ['blast', 'zap', 'copper', 'copper', 'silver'], played: ['blast', 'zap'], bought: ['zap'], produced: { damage: 3 } },
    { drawnHand: ['second-wind', 'storm', 'silver', 'copper', 'copper'], played: ['second-wind', 'storm'], bought: ['blast'], produced: { damage: 2, reattack: 1, stormTargets: 2 } },
    { drawnHand: ['zap', 'blast', 'silver', 'copper', 'copper'], played: ['zap', 'blast'], bought: ['silver'], produced: { damage: 3 } },
    { drawnHand: ['storm', 'zap', 'blast', 'silver', 'copper'], played: ['storm', 'zap', 'blast'], bought: ['gold'], produced: { damage: 5, stormTargets: 2 } },
    { drawnHand: ['zap', 'silver', 'silver', 'copper', 'rest'], played: ['zap'], bought: ['blast'], produced: { damage: 1 } }
  ],
  P2: [
...
const recruitPlans = {
  P1: ['scout', 'raider', 'marksman', 'scout', 'raider', 'marksman', 'guardian', 'scout'],
...
while (turnNumber <= 70) {
  ...
  applyUpgrades(player, plan.produced);
  attackWithUnits(player, plan.produced, actions);
  applyHealing(player, plan.produced);
...
await writeFile(join(root, 'timeline.json'), `${JSON.stringify({ schemaVersion: 1, title: 'E019 round 1 rush vs engine A strict', entries }, null, 2)}\n`);
...
function applyUpgrades(player, produced) {
...
function applyHealing(player, produced) {
  ...
  for (const healer of board.units.filter((unit) => unit.player === player && (unitRules[unit.type].heal ?? 0) > 0)) {
    const target = board.units
      .filter((unit) => unit.player === player && unit.hp < unit.maxHp && distance(healer, unit) <= unitRules[healer.type].range)
      .sort((a, b) => a.hp / a.maxHp - b.hp / b.maxHp)[0];
    if (target) {
      target.hp = Math.min(target.maxHp, target.hp + unitRules[healer.type].heal);
    }
  }
}
...
async function writeNotes() {
  ...
  const text = `# E019 Round 1 Rush Vs Engine A
...
- P2's engine/control line used healing, upgradeHealth, and center consolidation to stabilize. In this sample, ${winningPlayer === 'P2' ? 'that stabilization converted into the eventual unit-count lead after the rush thinned out.' : `${losingPlayer}'s response was not enough to prevent the eventual unit-count lead.`}
