...
const MAP_ID = 'sketch-v3-contest';
const TITLE = 'E019 R1 upgrade/support vs swarm/economy A strict';
...
const recruitPlans = {
  P1: ['guardian', 'healer', 'druid', 'marksman', 'guardian'],
...
const starterBoard = JSON.parse(await readFile(join(ROOT, '.games', 'e013-contest-starter.board.json'), 'utf8'));
...
for (let turnNumber = 1; turnNumber <= 44; turnNumber += 1) {
  ...
  applyUpgrades(board, player, cardPlan.produced, tacticalLog);
  resolveAttacks(boardBefore, board, player, cardPlan.produced, actions, tacticalLog);
  applyHealing(board, player, cardPlan.produced, tacticalLog);
...
if (!winner && completedTurns >= 40) {
  notes.push(`Stopped unresolved after ${completedTurns} completed player turns: no pending 4-unit lead survived the response window, and both sides still had legal but slow progress.`);
}
...
function applyUpgrades(state, player, produced, log) {
...
function applyHealing(state, player, produced, log) {
  ...
  while (heal > 0) {
    ...
    target.hp += 1;
    heal -= 1;
    log.push(`${player} used 1 deck heal on ${target.id}.`);
  }

  for (const healer of state.units.filter((unit) => unit.player === player && UNIT_RULES[unit.type].heal)) {
    ...
    if (target) {
      target.hp += 1;
      log.push(`${healer.id} used printed healing on ${target.id}.`);
    }
  }
}
...
function p1RecruitAllowance(state, amount) {
  ...
  if (p1Units <= 4 && amount >= 6) return 1;
  if (p1Units <= 5 && amount >= 12) return 1;
  if (amount >= 18 && p1Units <= 6) return 1;
  return 0;
}
...
async function writeNotes() {
  ...
  const lines = [
    '# E019 R1 Upgrade/Support vs Swarm/Economy A',
    '',
    ...
    '',
    'Legality notes:',
    '- Generated from the assigned E013 shell with explicit strict action logs for movements, recruits, and attacks.',
    ...
    '- P1 followed a low-recruit upgrade/support plan: Armory and Training repeatedly strengthened the original guardian anchor while Bandage/Potion healing kept damaged bodies relevant.',
    ...
    '- The important tension was quantity versus quality. P1 produced durable units, but P2 converted center pressure into more bodies and created the decisive response-window lead before turn 40.',
    '',
    'Win-window log:',
    ...notes.map((note) => `- ${note}`)
  ];
