...
const runDir = '.games/e017a-upgrade-vs-swarm-a';
...
const starterBoard = '.games/e017-centercontrol-tight-starter.board.json';
...
function applyBoardTurn(board: BoardState, map: any, hexes: Set<string>, player: Player, produced: Record<string, number>): { summary: string; reasoning: string } {
  ...
  const reasoning = `${player === 'P1'
    ? 'P1 followed the upgrade/support assignment by prioritizing Armory/Training/Second Wind value, healing damaged quality units, and contesting eastern plus central centers.'
    : 'P2 followed the swarm/economy assignment by buying Silver/Gold/Peddler/Village, recruiting broadly, and sending fast units to flip or pressure center-control thresholds.'} Board events: ${events.join('; ')}.`;
...
function centerControlCondition(board: BoardState, player: Player, completedTurnsSoFar: number): boolean {
  return completedTurnsSoFar >= 16
    && centersControlled(board, player) >= 5
    && liveUnits(board, player).length - liveUnits(board, other(player)).length >= 2;
}
...
async function main(): Promise<void> {
  ...
  const timeline: { schemaVersion: 1; title: string; entries: ReplayEntry[] } = {
    schemaVersion: 1,
    title: 'E017a upgrade/support vs swarm/economy A tight center-control replay',
    entries: []
  };
  seedCounters(board);
  ...
  for (let turnNumber = 1; turnNumber <= maxCompletedTurns; turnNumber += 1) {
    ...
    if (pendingCenterThreat && !centerControlCondition(board, pendingCenterThreat, completedTurns)) {
      startThreatEvents.push(`cleared pending ${pendingCenterThreat} center-control before ${player}'s start check (${threatState(board)})`);
    ...
    if (pendingCenterThreat === player && centerControlCondition(board, player, completedTurns)) {
      winner = player;
      winType = 'confirmed center-control response-window win';
      threatLog.push(`Turn ${turnNumber}: ${player} confirmed pending center-control at start of turn (${threatState(board)}).`);
      break;
    }
    ...
    if (centerControlCondition(board, player, completedTurns) && pendingCenterThreat !== player) {
      pendingCenterThreat = player;
      startThreatEvents.push(`${player} created pending center-control (${threatState(board)})`);
    }
    ...
    const pendingText = [
      ...
      pendingCenterThreat ? `pending center-control: ${pendingCenterThreat}` : ''
  ...
  const notes = `# E017a Upgrade/Support vs Swarm/Economy A Notes
