...
const RULESET = 'territory-v1-cost6-damagecap-responsewin-lead4-highmove-center6-tight';
const MAP_ID = 'sketch-v5-recenter';
const TITLE = 'E024b R1 upgrade/support vs swarm/economy A strict high-movement six-center tight deficit';
...
const recruitPlans = {
  P1: ['guardian', 'scout', 'marksman', 'raider', 'healer', 'druid', 'scout', 'guardian', 'marksman', 'raider'],
...
const starterBoard = JSON.parse(await readFile(join(ROOT, '.games', 'e024b-highmove-center6-tight-starter.board.json'), 'utf8'));
...
let pendingUnitThreat = null;
let pendingCenterThreat = null;
let winner = null;
let winnerType = null;
...
for (let turnNumber = 1; turnNumber <= 160; turnNumber += 1) {
  ...
  const startEvents = [];
  const pendingLead = pendingUnitThreat ? countUnits(board, pendingUnitThreat.player) - countUnits(board, other(pendingUnitThreat.player)) : 0;

  if (pendingUnitThreat?.player === player && pendingLead >= 4) {
    winner = player;
    winnerType = 'unit-count';
    notes.push(`${player} confirmed a pending 4-unit lead at the start of turn ${turnNumber}; no extra board phase was recorded.`);
    break;
  }
  if (pendingUnitThreat && pendingLead < 4) {
    const event = `${pendingUnitThreat.player}'s pending unit-count threat was cleared before turn ${turnNumber}; ${pendingUnitThreat.player} now leads by ${pendingLead}.`;
    notes.push(event);
    startEvents.push(event);
    pendingUnitThreat = null;
  }
  if (!pendingUnitThreat && unitLead >= 4) {
    pendingUnitThreat = { player, afterTurn: completedTurns };
    const event = `${player} created a pending unit-count threat at the start of turn ${turnNumber}: ${countUnits(board, player)} vs ${countUnits(board, opponent)}.`;
    notes.push(event);
    startEvents.push(event);
  }

  const centerEvent = checkSixCenterThreat(board, player, turnNumber, completedTurns, startEvents);
  if (centerEvent?.winner) {
    winner = centerEvent.winner;
    winnerType = 'six-center-dominance';
    notes.push(`${winner} confirmed a pending late six-center dominance threat at the start of turn ${turnNumber}; no extra board phase was recorded.`);
    break;
  }
...
if (!winner && completedTurns >= 40) {
  notes.push(`Stopped unresolved after ${completedTurns} completed player turns: no pending unit-count or late six-center dominance threat survived the response window, home-base congestion limited further recruitment, and repeated legal attacks/healing no longer changed the unit-count or center-control gap enough to create a plausible forced finish.`);
}
...
function p1RecruitAllowance(state, amount) {
  ...
  if (p1Units < p2Units) return Math.min(affordable, p2Units - p1Units + 1);
  if (p1Units <= p2Units + 2) return Math.min(affordable, amount >= 12 ? 2 : 1);
  if (amount >= 12 && p1Units <= p2Units + 4) return 1;
  return 0;
}
...
function checkSixCenterThreat(state, player, turnNumber, turnsCompleted, startEvents) {
  if (pendingCenterThreat) {
    const pendingPlayer = pendingCenterThreat.player;
    const stillThreatening = hasSixCenterThreat(state, pendingPlayer);
    if (pendingPlayer === player) {
      if (stillThreatening) {
        return { winner: player };
      }
      const event = `${pendingPlayer}'s pending late six-center dominance threat was cleared before turn ${turnNumber}: ${countCenters(state, pendingPlayer)} centers and unit count ${countUnits(state, pendingPlayer)} vs ${countUnits(state, other(pendingPlayer))}.`;
      notes.push(event);
      startEvents.push(event);
      pendingCenterThreat = null;
    } else if (!stillThreatening) {
      const event = `${pendingPlayer}'s pending late six-center dominance threat was cleared before turn ${turnNumber}: ${countCenters(state, pendingPlayer)} centers and unit count ${countUnits(state, pendingPlayer)} vs ${countUnits(state, other(pendingPlayer))}.`;
      notes.push(event);
      startEvents.push(event);
      pendingCenterThreat = null;
  ...
  if (!pendingCenterThreat && turnsCompleted >= 18 && hasSixCenterThreat(state, player)) {
    pendingCenterThreat = { player, afterTurn: turnsCompleted };
    const event = `${player} created a pending late six-center dominance threat at the start of turn ${turnNumber}: ${countCenters(state, player)} centers and unit count ${countUnits(state, player)} vs ${countUnits(state, other(player))}.`;
    notes.push(event);
    startEvents.push(event);
  }
...
async function writeNotes() {
  ...
  const lines = [
    '# E024b R1 Upgrade/Support vs Swarm/Economy A',
    '',
    `Result: ${winner ? `${winner} wins by confirmed ${winnerType === 'six-center-dominance' ? 'late six-center dominance response-window win' : 'lead-4 response-window unit-count win'}` : 'unresolved'}.`,
    ...
    '',
    'Legality notes:',
    '- Generated from the assigned E024b high-movement six-center-tight starter with explicit strict action logs for movements, recruits, attacks, healing, and permanent upgrades.',
    ...
    '- P1 followed an upgrade/support plan with a round-2 emphasis on active center denial: Armory and Training created anchors while saved supply was spent on scouts, marksmen, and support bodies to contest six-center lines.',
    ...
    winner
      ? '- The important tension was whether late six-center dominance could resolve a support-versus-swarm replacement loop below 50 turns and whether the tighter E024b unit-deficit clear condition prevented arbitrary six-center conversion.'
      : '- The important tension was whether late six-center dominance could resolve a support-versus-swarm replacement loop below 50 turns and whether the tighter E024b unit-deficit clear condition prevented arbitrary six-center conversion.',
    '',
    'Win-window log:',
    ...notes.map((note) => `- ${note}`)
...
function countCenters(state, player) {
  return state.supplyControl.filter((center) => center.controller === player).length;
}

function hasSixCenterThreat(state, player) {
  return countCenters(state, player) >= 6 && countUnits(state, player) >= countUnits(state, other(player)) - 1;
}
