...
const root = '.games/e021r1-rush-engine-a';
const map = JSON.parse(await readFile('maps/sketch-v5-recenter.json', 'utf8'));
const initialBoard = JSON.parse(await readFile('.games/e021-recenter-no-printheal-starter.board.json', 'utf8'));
...
await writeFile(join(root, 'timeline.json'), `${JSON.stringify({ schemaVersion: 1, title: 'E021 round 1 rush vs engine A strict recenter no printed healing', entries }, null, 2)}\n`);
...
async function writeNotes() {
  ...
  const text = `# E021 Round 1 Rush Vs Engine A
