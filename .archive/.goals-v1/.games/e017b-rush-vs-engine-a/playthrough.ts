...
const RUN = '.games/e017b-rush-vs-engine-a';
...
const p1Targets: Record<UnitType, string[]> = {
  scout: ['center-northeast', 'center-east', 'center-southeast', 'center-center', 'center-center-north', 'center-center-south'],
  raider: ['center-northeast', 'center-center', 'center-east', 'center-center-north', 'center-southeast', 'center-center-south'],
  guardian: ['center-center', 'center-center-north', 'center-east', 'center-northeast', 'center-southeast'],
  marksman: ['center-northeast', 'center-center-north', 'center-center', 'center-east', 'center-southeast'],
...
const p2Targets: Record<UnitType, string[]> = {
  scout: ['center-west-south', 'center-center-south', 'center-center', 'center-southeast', 'center-east', 'center-northwest'],
  raider: ['center-center-south', 'center-center', 'center-west-south', 'center-southeast', 'center-northwest'],
  marksman: ['center-west-south', 'center-center-south', 'center-center', 'center-southeast', 'center-east'],
  guardian: ['center-center-south', 'center-center', 'center-west-south', 'center-northwest', 'center-southeast'],
...
const p1Recruits: UnitType[] = ['raider', 'scout', 'marksman', 'raider', 'scout', 'marksman', 'guardian', 'raider'];
const p2Recruits: UnitType[] = ['guardian', 'healer', 'druid', 'marksman', 'guardian', 'healer', 'druid', 'marksman', 'scout'];
...
async function main(): Promise<void> {
  ...
  board.notes = [...(board.notes ?? []), 'E017b rush-vs-engine-a generated full-game run; tight center-control response window and normal 2 + centers income used every board phase.'];
  ...
  for (let turnNumber = 1; turnNumber <= maxTurns && !winner; turnNumber += 1) {
    ...
    if (centerEligible) {
      if (pendingCenter === player) {
        winner = player;
        startMessages.push(`${player} confirmed pending tight center-control threat with ${centers[player]} centers and unit count ${counts[player]}-${counts[opponent]}.`);
        notes.push(`Turn ${turnNumber} ${player} start: ${startMessages.at(-1)}`);
        break;
      }
      pendingCenter = player;
      startMessages.push(`${player} created pending tight center-control threat with ${centers[player]} centers and unit count ${counts[player]}-${counts[opponent]}.`);
      notes.push(`Turn ${turnNumber} ${player} start: ${startMessages.at(-1)}`);
      ...
      if (threatCenters < 5 || counts[threatened] - counts[defender] < 2) {
        startMessages.push(`Pending ${threatened} tight center-control threat cleared at centers P1 ${centers.P1} / P2 ${centers.P2} and units P1 ${counts.P1} / P2 ${counts.P2}.`);
        notes.push(`Turn ${turnNumber} ${player} start: ${startMessages.at(-1)}`);
        pendingCenter = null;
      }
    }
    ...
    if (counts[player] - counts[opponent] >= 4) {
      if (pendingLead === player) {
        winner = player;
        startMessages.push(`${player} confirmed pending lead-4 threat at ${counts[player]}-${counts[opponent]}.`);
        notes.push(`Turn ${turnNumber} ${player} start: ${startMessages.at(-1)}`);
        break;
      }
      pendingLead = player;
      startMessages.push(`${player} created pending lead-4 threat at ${counts[player]}-${counts[opponent]}.`);
      notes.push(`Turn ${turnNumber} ${player} start: ${startMessages.at(-1)}`);
    } else if (pendingLead && counts[pendingLead] - counts[pendingLead === 'P1' ? 'P2' : 'P1'] < 4) {
      startMessages.push(`Pending ${pendingLead} lead threat cleared at P1 ${counts.P1} / P2 ${counts.P2}.`);
      notes.push(`Turn ${turnNumber} ${player} start: ${startMessages.at(-1)}`);
      pendingLead = null;
    }
    if (startMessages.length === 0) {
      startMessages.push(`No center-control or unit-count threat changed at start; centers P1 ${centers.P1} / P2 ${centers.P2}, units P1 ${counts.P1} / P2 ${counts.P2}.`);
    }
    ...
    entries.push({
      ...
      reasoning: [`Start check: ${startMessages.join(' ')}`, boardRecord.reasoning].join(' ')
    });
  }

  const finalCounts = unitCounts(board);
  const centerSplit = countCenters(board);
  notes.unshift(`# E017b Rush vs Engine A Notes`, '');
  ...
  notes.push('- Assigned tension: P1 rush tests whether E017 blocks the E016-style easy center-control rush while P2 engine/healing gets one response turn to flip centers or cut the unit lead.');
  ...
  await writeJson(join(RUN, 'timeline.json'), { schemaVersion: 1, title: 'E017b Rush vs Engine A', entries });
...
function playDeckTurn(deck: { schemaVersion: 1; rngState: number; game: GameState }, rng: SeededRng, player: Player): TurnRecord['deck'] {
  ...
  const actionPriority = player === 'P1'
    ? ['village', 'peddler', 'zap', 'bandage', 'blast', 'storm', 'second-wind', 'training']
    : ['village', 'peddler', 'bandage', 'potion', 'armory', 'healer', 'zap'];
  let changed = true;
  while (changed) {
    changed = false;
...
function centerControlConversionMove(board: BoardState, player: Player, unit: Unit, candidates: Coord[]): Coord | undefined {
  ...
  const units = unitCounts(board);
  const opponent = player === 'P1' ? 'P2' : 'P1';
  const canConvert = player === 'P1'
    ? centers[player] >= 3 && units[player] >= units[opponent] - 1
    : centers.P1 >= 5 || centers[player] === 4;
  if (!canConvert) {
    return undefined;
  }
...
function scoreCoord(board: BoardState, player: Player, unit: Unit, coord: Coord, targets: string[]): number {
  ...
  for (let index = 0; index < targets.length; index += 1) {
    ...
    const controlPenalty = control?.controller === player ? 90 : 0;
    const opponentBonus = control?.controller === opponent ? -18 : 0;
    const neutralBonus = control?.controller === null ? -12 : 0;
    score = Math.min(score, distance(coord, target) * 10 + index * 18 + controlPenalty + opponentBonus + neutralBonus);
  }
  return score + (enemyNear ? -8 : 0);
}
