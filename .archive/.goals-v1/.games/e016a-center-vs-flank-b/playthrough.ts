...
const RUN = '.games/e016a-center-vs-flank-b';
const MAP_PATH = 'maps/sketch-v3-contest.json';
const BOARD_PATH = '.games/e016-centercontrol-starter.board.json';
...
async function main(): Promise<void> {
  ...
  board.notes = [...(board.notes ?? []), 'E016a center-vs-flank-b generated full-game run; normal 2 + centers income used every board phase.'];
  ...
  for (let turnNumber = 1; turnNumber <= maxTurns && !winner; turnNumber += 1) {
    ...
    if (centers[player] >= 5 && counts[player] >= counts[opponent]) {
      if (pendingCenter === player) {
        winner = player;
        notes.push(`Turn ${turnNumber} ${player} start: ${player} confirmed pending center-control threat with ${centers[player]} centers and unit count ${counts[player]}-${counts[opponent]}.`);
        break;
      }
      pendingCenter = player;
      notes.push(`Turn ${turnNumber} ${player} start: ${player} created pending center-control threat with ${centers[player]} centers and unit count ${counts[player]}-${counts[opponent]}.`);
      ...
      if (threatCenters < 5 || counts[threatened] < counts[defender]) {
  ...
  notes.unshift(`# E016a Center vs Flank B Notes`, '');
  ...
  await writeJson(join(RUN, 'timeline.json'), { schemaVersion: 1, title: 'E016a Center vs Flank B', entries });
