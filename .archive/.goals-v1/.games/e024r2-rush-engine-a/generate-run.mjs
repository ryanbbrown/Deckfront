...
const root = '.games/e024r2-rush-engine-a';
const map = JSON.parse(await readFile('maps/sketch-v5-recenter.json', 'utf8'));
const initialBoard = JSON.parse(await readFile('.games/e024-highmove-center6-starter.board.json', 'utf8'));
...
while (turnNumber <= 70) {
  ...
  if (entries.length >= 18 && centerCounts[player] >= 6 && count[opponent] - count[player] < 3 && pendingCenter?.player === player) {
    winner = player;
  ...
  if (entries.length >= 18) {
    if (centerCounts[player] >= 6 && count[opponent] - count[player] < 3) {
      ...
      (centerCounts[opponent] < 6 || count[player] - count[opponent] >= 3)
    ) {
      startThreatEvents.push(
...
await writeFile(join(root, 'timeline.json'), `${JSON.stringify({ schemaVersion: 1, title: 'E024 round 2 rush vs engine A strict high-movement six-center dominance', entries }, null, 2)}\n`);
...
async function writeNotes() {
  ...
  const text = `# E024 Round 2 Rush Vs Engine A
...
- Movement endpoints, one-unit-per-hex occupancy, blocked/off-map placement, attack range, damage accounting, delayed recruit attacks, damage-cap use, healing, and permanent stat changes are intended to be checked by \`bun run validate-run -- --strict .games/e024r2-rush-engine-a/timeline.json\`.
...
- P1's sharper round-2 rush prioritized high-movement northeast/east pressure, scout/raider bodies, earlier Second Wind, and damage cards only when legal attack carriers existed.
