...
const root = '.games/e023r1-rush-engine-a';
const map = JSON.parse(await readFile('maps/sketch-v5-recenter.json', 'utf8'));
const initialBoard = JSON.parse(await readFile('.games/e023-highmove-centermid-starter.board.json', 'utf8'));
...
while (turnNumber <= 70) {
  ...
  if (entries.length >= 24 && centerCounts[player] >= 5 && count[player] >= count[opponent] && pendingCenter?.player === player) {
    winner = player;
    winType = 'center-majority';
    winReason = `${player} confirms a late center-majority win at start of ${turnId} with ${centerCounts[player]} centers and units ${count[player]} to ${count[opponent]}.`;
    break;
  }
  ...
  if (entries.length >= 24) {
    if (centerCounts[player] >= 5 && count[player] >= count[opponent]) {
      if (pendingCenter?.player !== player) {
        startThreatEvents.push(
          `${player} records a pending late center-majority threat at start of ${turnId} (${centerCounts[player]} centers, units ${count[player]}-${count[opponent]}).`
        );
      }
      pendingCenter = { player };
    } else if (
      pendingCenter?.player === opponent &&
      (centerCounts[opponent] < 5 || count[player] > count[opponent])
    ) {
      startThreatEvents.push(
        `${player} clears ${opponent}'s pending late center-majority threat at start of ${turnId} (${centerCounts[opponent]} ${opponent} centers, units ${count[player]}-${count[opponent]}).`
      );
      pendingCenter = null;
    }
  }
  ...
  const centerStatus =
    entries.length + 1 < 24
      ? `Late center-majority is not live yet (${entries.length + 1} completed player turns after this entry).`
      : `Late center-majority is live after this entry; centers are P1 ${afterCenters.P1}, P2 ${afterCenters.P2}, neutral ${afterCenters.neutral}, units P1 ${afterCounts.P1}, P2 ${afterCounts.P2}.`;
  const pendingStatus = `Pending threats after this turn: unit-count ${pendingLead?.player ?? 'none'}; center-majority ${pendingCenter?.player ?? 'none'}.`;
  const reasoning = [
    ...startThreatEvents,
...
await writeFile(join(root, 'timeline.json'), `${JSON.stringify({ schemaVersion: 1, title: 'E023 round 1 rush vs engine A strict high-movement late center-majority', entries }, null, 2)}\n`);
...
async function writeNotes() {
  ...
  const text = `# E023 Round 1 Rush Vs Engine A

Result: ${winningPlayer}${winReason ? ` by confirmed ${winType === 'center-majority' ? 'late center-majority' : 'lead-4 unit-count'} response-window win.` : '.'}
...
- Starter board: .games/e023-highmove-centermid-starter.board.json

Legality notes:

- Every timeline entry includes actions.movements, actions.recruits, actions.attacks, actions.heals, and actions.upgrades.
- Movement endpoints, one-unit-per-hex occupancy, blocked/off-map placement, attack range, damage accounting, delayed recruit attacks, damage-cap use, healing, and permanent stat changes are intended to be checked by \`bun run validate-run -- --strict .games/e023r1-rush-engine-a/timeline.json\`.
...
- Late center-majority threats are checked only at start-of-turn after at least 24 completed player turns. This run records pending/cleared center-majority state in each timeline entry's reasoning. In this sample, the game resolved by ${winType === 'center-majority' ? 'late center-majority before unit-count confirmation.' : 'unit count before any center-majority confirmation.'}
...
- Under current strict logging, this is evidence for the E023 high-movement late center-majority branch.
`;
