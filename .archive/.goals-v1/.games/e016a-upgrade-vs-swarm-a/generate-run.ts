...
const runDir = '.games/e016a-upgrade-vs-swarm-a';
...
const starterBoard = '.games/e016-centercontrol-starter.board.json';
...
function centerControlCondition(board: BoardState, player: Player): boolean {
...
async function main(): Promise<void> {
  ...
  const timeline: { schemaVersion: 1; title: string; entries: ReplayEntry[] } = {
    schemaVersion: 1,
    title: 'E016a upgrade/support vs swarm/economy A center-control replay',
    entries: []
  };
  seedCounters(board);
  ...
  for (let turnNumber = 1; turnNumber <= maxCompletedTurns; turnNumber += 1) {
    ...
    if (pendingCenterThreat && !centerControlCondition(board, pendingCenterThreat)) {
    ...
    if (pendingCenterThreat === player && centerControlCondition(board, player)) {
      winner = player;
    ...
    if (centerControlCondition(board, player) && pendingCenterThreat !== player) {
  ...
  const notes = `# E016a Upgrade/Support vs Swarm/Economy A Notes
