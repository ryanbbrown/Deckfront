# Territory V1 Cost 6 Damage Cap Response Win Lead 4 Center Control Rules

This is a center-control pressure variant of `territory-v1-cost6-damagecap-responsewin-lead4` for E016 playtests. It keeps the same cards, units, timing, recruit cost 6, normal center income, deck-damage cap, and lead-4 response-window unit-count win. It adds a second response-window win condition for sustained center control.
...
- Deck config: `rulesets/territory-v1-cost6-damagecap-responsewin-lead4-centercontrol/deck.yaml`
...
Do not use the older 7-Copper draft-start prose from prior rulesets for E016 runs. If a playtest uses a different initial setup, record that in the run notes and downgrade that run's evidence quality.
...
- Confirmed 5-center control while not behind on living units.
...
Center-control confirmation also works as a response window:

1. At the beginning of a player's turn, count controlled supply centers and living units.
2. If the active player controls at least 5 supply centers, has at least as many living units as the opponent, and already had that same pending center-control threat after the opponent's previous completed turn, the active player wins immediately.
3. If the active player controls at least 5 supply centers and has at least as many living units as the opponent, but the threat is not already pending, record a pending center-control win threat for that player and continue the turn normally.
4. The opponent then gets a full turn to reduce the active player's control below 5 centers or move ahead on living units.
5. If either condition is broken before the threatened player's next start-of-turn check, the pending center-control threat is cleared.

Examples:

- P1 starts a turn controlling 5 centers with units tied 8-8 and no pending center threat: P1 records a pending center-control threat, but does not win yet.
- P2 then flips one center, making P1 control only 4 centers: the pending P1 center-control threat is cleared.
- P2 instead kills two P1 units, making P1 trail 6-8 while still controlling 5 centers: the pending P1 center-control threat is cleared.
- P1 later starts a turn controlling 5 centers and leading or tied on living units, after P2 failed to break the prior pending threat: P1 wins.
