# Territory V1 Cost 6 Damage Cap Response Win Lead 4 No Printed Healing Rules

This is a combat/support durability variant of `territory-v1-cost6-damagecap-responsewin-lead4` for E018 playtests. It keeps the same cards, map, timing, recruit cost 6, normal income, deck-damage cap, and lead-4 response-window win condition. It removes printed healing from units while leaving deck-produced healing and upgrade-health healing intact.
...
- Deck config: `rulesets/territory-v1-cost6-damagecap-responsewin-lead4-no-printheal/deck.yaml`
...
Do not use the older 7-Copper draft-start prose from prior rulesets for E018 runs. If a playtest uses a different initial setup, record that in the run notes and downgrade that run's evidence quality.
...
Units have no printed healing in E018. Druids and healers keep their unit bodies, attack values, movement, and range, but their printed `heal` value is treated as 0 and they cannot heal instead of attacking.

Deck-produced `heal` counters and `upgradeHealth` healing still work normally. This experiment removes only repeatable unit-board healing to test whether guardian/healer/druid support loops are driving long attrition games.
