# Territory V1 Cost 6 Damage Cap Response Win Lead 4 Tight Center Control Rules

This is a tightened center-control pressure variant of `territory-v1-cost6-damagecap-responsewin-lead4` for E017 playtests. It keeps the same cards, units, timing, recruit cost 6, normal center income, deck-damage cap, and lead-4 response-window unit-count win. It adds a stricter response-window win condition for sustained center control.
...
- Deck config: `rulesets/territory-v1-cost6-damagecap-responsewin-lead4-centercontrol-tight/deck.yaml`
...
1. At the beginning of the active player's turn, check whether that player wins by a confirmed response-window threat, as defined in Win Conditions.
...
The initial deck is fixed by `deck.yaml`: each player starts with 5 Copper, 1 Zap, 1 Bandage, and 3 Rest.

Do not use the older 7-Copper draft-start prose from prior rulesets for E017 runs. If a playtest uses a different initial setup, record that in the run notes and downgrade that run's evidence quality.

## Win Conditions

A player wins at the beginning of their turn if they satisfy either confirmed response-window win condition:

- Confirmed 4-unit lead.
- Confirmed tight center control.
...
Tight center-control confirmation also works as a response window:

1. At the beginning of a player's turn, count completed player turns, controlled supply centers, and living units.
2. A center-control threat cannot be created before turn 17. In practice, this means at least 16 completed player turns must already be recorded in the replay timeline before a start-of-turn center-control check can create or confirm a center-control threat.
3. If the active player controls at least 5 supply centers, has at least 2 more living units than the opponent, is at or after turn 17, and already had that same pending center-control threat after the opponent's previous completed turn, the active player wins immediately.
4. If the active player controls at least 5 supply centers, has at least 2 more living units than the opponent, and is at or after turn 17, but the threat is not already pending, record a pending center-control win threat for that player and continue the turn normally.
5. The opponent then gets a full turn to reduce the active player's control below 5 centers or reduce the active player's living-unit lead below 2.
6. If either condition is broken before the threatened player's next start-of-turn check, the pending center-control threat is cleared.

Examples:

- P1 starts turn 15 controlling 6 centers and leading 8-6 on units: P1 does not create a center-control threat because the turn gate has not opened.
- P1 starts turn 17 controlling 5 centers and leading 8-6 on units with no pending center threat: P1 records a pending center-control threat, but does not win yet.
- P2 then flips one center, making P1 control only 4 centers: the pending P1 center-control threat is cleared.
- P2 instead kills one P1 unit, making P1 lead only 7-6 while still controlling 5 centers: the pending P1 center-control threat is cleared.
- P1 later starts a turn controlling 5 centers and leading by at least 2 living units, after P2 failed to break the prior pending threat: P1 wins.

The board schema has no field for pending center-control threats. During playtests, record pending center threats in the relevant `timeline.json` reasoning and in `notes.md`.
...
Movement uses the active map's coordinate system. `maps/sketch-v3-contest.json` uses flat-top, odd-column offset coordinates:
...
- Whether the fixed `deck.yaml` start should become the permanent setup for future locked rulesets.
