# Territory V1 Cost 6 Damage Cap Response Win Lead 4 High Movement Center-Mid Rules

This is a high-movement anti-grind variant of `territory-v1-cost6-damagecap-responsewin-lead4` for E023 playtests. It keeps the same cards, timing, recruit cost 6, normal income, deck-damage cap, high movement values, and lead-4 response-window unit-count win condition. It adds a late center-majority response-window win to test whether sustained territory control can resolve long replacement grinds without ending on early center spikes.

## Source Files

- Map: `maps/sketch-v5-recenter.json`
- Sample run state: `.games/territory-v1-playtest/board.json`
- Units: `rulesets/territory-v1-cost6-damagecap-responsewin-lead4-highmove-centermid/units.json`
- Deck config: `rulesets/territory-v1-cost6-damagecap-responsewin-lead4-highmove-centermid/deck.yaml`
...
1. At the beginning of the active player's turn, check whether that player wins by a confirmed unit-count lead or confirmed late center-majority threat, as defined in Win Condition.
...
The initial deck is fixed for this high-movement center-majority variant: each player starts with 7 Coppers plus up to 12 coin to draft additional starting cards before turn 1. Unspent draft coin does not carry over in E023 runs.
...
A player wins at the beginning of their turn if they have a confirmed 4-unit lead or a confirmed late center-majority threat.
...
### Late Center-Majority

Late center-majority confirmation also works as a response window:

1. At the beginning of a player's turn, after at least 24 completed player turns have already been recorded, count controlled supply centers and living units.
2. If the active player controls at least 5 of the 8 supply centers and is not behind on living units, record a pending center-majority win threat for that player if one is not already pending.
3. If the active player already had that same pending center-majority threat after the opponent's previous completed turn, and still controls at least 5 centers while not behind on living units, the active player wins immediately.
4. The opponent gets a full turn to clear the threat by reducing the threatened player's control below 5 centers or moving ahead on living units.
5. If either clear condition is met before the threatened player's next start-of-turn check, the pending center-majority threat is cleared.

Examples:

- P2 starts turn 27 controlling 5 centers with units tied 10-10 and no pending center-majority threat: P2 records a pending threat, then continues the turn.
- P1 then flips one center so P2 controls only 4 centers: the pending P2 center-majority threat is cleared.
- P2 later starts a turn still controlling 5 centers and not behind on units after P1 failed to clear the prior pending threat: P2 wins.
- Before 24 completed player turns, 5 centers never creates a center-majority threat. Early high-water center spikes must convert through normal play.

## Units And Starting HP

Units start at their max HP and use the high-movement values from `rulesets/territory-v1-cost6-damagecap-responsewin-lead4-highmove-centermid/units.json`.
